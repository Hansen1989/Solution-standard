...
{
    public partial class PREPARE00List : PageBase
    {
        ...
        public void SearchOrder()
        {
            ...
            PREPARE00Bll.GetInstence().BindGrid(Grid1, 0, 0, InquiryCondition(), sortList);
        ...
        private List<ConditionFun.SqlqueryCondition> InquiryCondition()
        {
            ...
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, PREPARE00Table.INPUT_DATE, Comparison.LessOrEquals, et, false, false));
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PREPARE00Table.INPUT_DATE, Comparison.GreaterOrEquals, st, false, false));
            }
            else
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, PREPARE00Table.APP_DATETIME, Comparison.LessOrEquals, et, false, false));
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PREPARE00Table.APP_DATETIME, Comparison.GreaterOrEquals, st, false, false));
            }

            if (!sn.Equals("0") && !String.IsNullOrEmpty(sn))
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PREPARE00Table.SHOP_ID, Comparison.Equals, sn, false, false));
            }
        ...
        {
            string _tbxPREPAR_ID = GridViewHelper.GetSelectedKey(Grid1, true);
            tbxPREPAR_ID.Text = _tbxPREPAR_ID;
            LoadMAIN();
            LoadDETAIL();
        ...
        {
            string _tbxtbxPREPAR_ID = tbxPREPAR_ID.Text;
            if (!String.IsNullOrEmpty(_tbxtbxPREPAR_ID))
            {
                var model = new PREPARE00(x => x.PREPAR_ID == _tbxtbxPREPAR_ID);
                ...
                ddlPRE_TYPE.SelectedValue = model.PRE_TYPE.ToString();
                ...
                ckLOCKED.Checked = model.LOCKED ? true : false;
        ...
        public void LoadDETAIL()
        {
            ...
            if (!String.IsNullOrEmpty(_tbxPREPAR_ID))
            {
                ...
                conditiondetail.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, PREPARE01Table.PREPARE_ID, Comparison.Equals, _tbxPREPAR_ID, false, false));
                PREPARE01Bll.GetInstence().BindGrid(Grid2, 0, 0, conditiondetail, sortList);
            }
        }
        /// <summary>
        /// 状态位的判定
        ...
        public void BtnAdd_Click(Object sender, EventArgs e)
        {
            ...
            ddlPRE_TYPE.Enabled = true;
            ddlPRE_TYPE.SelectedIndex = 0;
        ...
        {
            string _tbxPREPAR_ID = tbxPREPAR_ID.Text.ToString();
            var model = PREPARE00.SingleOrDefault(x => x.PREPAR_ID == _tbxPREPAR_ID);
            if (model == null)
            {
        ...
        public string MAINEdit()
        {
            ...
            try
            {
                var model = new PREPARE00(x => x.PREPAR_ID == _tbxPREPAR_ID);
                ...
                if (String.IsNullOrEmpty(_tbxPREPAR_ID))
                {
                    ...
                    dt = (DataTable)SPs.Get_ORDER_SEED(_SHOP_ID, "PREPARE00").ExecuteDataTable();
                    _tbxPREPAR_ID = dt.Rows[0]["PLANE_ID"].ToString();
                    //var model = Purchase00.SingleOrDefault(x => x.Purchase_ID == _Pur00_id);

                }
                model.SHOP_ID = _SHOP_ID;
                model.PREPAR_ID = _tbxPREPAR_ID.ToString();
                ...
                model.PRE_TYPE = ConvertHelper.Cint(ddlPRE_TYPE.SelectedValue);
                ...
                model.LOCKED = ckLOCKED.Checked ? true : false;

                model.MOD_DATETIME = DateTime.Now;
                model.MOD_USER_ID = OlUser.Manager_LoginName;
                model.LAST_UPDATE = DateTime.Now;
                model.Trans_STATUS = false;
                PREPARE00Bll.GetInstence().Save(this, model);
            }
            catch (Exception err)
            {
                return err.Message;
