...
{
    public partial class Dispose00List : PageBase
    {
        ...
        public void SearchOrder()
        {
            ...
            Dispose00Bll.GetInstence().BindGrid(Grid1, 0, 0, InquiryCondition(), sortList);
        ...
        private List<ConditionFun.SqlqueryCondition> InquiryCondition()
        {
            ...
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, Dispose00Table.INPUT_DATE, Comparison.LessOrEquals, et, false, false));
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, Dispose00Table.INPUT_DATE, Comparison.GreaterOrEquals, st, false, false));
            }
            else
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, Dispose00Table.APP_DATETIME, Comparison.LessOrEquals, et, false, false));
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, Dispose00Table.APP_DATETIME, Comparison.GreaterOrEquals, st, false, false));
            }

            if (!sn.Equals("0") && !String.IsNullOrEmpty(sn))
            {
                conditionList.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, Dispose00Table.SHOP_ID, Comparison.Equals, sn, false, false));
            }
        ...
        {
            string _tbxDP_ID = GridViewHelper.GetSelectedKey(Grid1, true);
            tbxDP_ID.Text = _tbxDP_ID;
            LoadMAIN();
            LoadDETAIL();
        ...
        {
            string _tbxDP_ID = tbxDP_ID.Text;
            if (!String.IsNullOrEmpty(_tbxDP_ID))
            {
                var model = new Dispose00(x => x.DP_ID == _tbxDP_ID);
                ...
                ddlDP_TYPE.SelectedValue = model.DP_TYPE.ToString();
        ...
        public void LoadDETAIL()
        {
            ...
            if (!String.IsNullOrEmpty(_tbxDP_ID))
            {
                ...
                conditiondetail.Add(new ConditionFun.SqlqueryCondition(ConstraintType.Where, Dispose00Table.DP_ID, Comparison.Equals, _tbxDP_ID, false, false));
                Material01Bll.GetInstence().BindGrid(Grid2, 0, 0, conditiondetail, sortList);
            }
        }
        /// <summary>
        /// 状态位的判定
        ...
        public void BtnAdd_Click(Object sender, EventArgs e)
        {
            ...
            ddlDP_TYPE.Enabled = true;
            ddlDP_TYPE.SelectedIndex = 0;
        ...
        {
            string _DP_ID = tbxDP_ID.Text.ToString();
            var model = Dispose00.SingleOrDefault(x => x.DP_ID == _DP_ID);
            if (model == null)
            {
        ...
        public string MAINEdit()
        {
            ...
            try
            {
                ...
                model.DP_TYPE = ConvertHelper.Cint(ddlDP_TYPE.SelectedValue);
                ...
                Dispose00Bll.GetInstence().Save(this, model);
            }
            catch (Exception err)
            {
                return err.Message;
        ...
        public string DetailEdit()
        {
            ...
            for (int i = 0; i < jarr.Count; i++)
            {
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(jarr[i]["values"]["DP_ID01"].ToString()))
                    {
                        model2.DP_ID = jarr[i]["values"]["DP_ID01"].ToString();
                    }
                    else
                    {
                        return "保存失败";
                    ...
                    model2.STD_QUAN = ConvertHelper.Cdbl(jarr[i]["values"]["STD_QUAN01"].ToString());
                    ...
                    Dispose01Bll.GetInstence().Save(this, model2);
                }
                catch (Exception err)
                {
                    n++;
