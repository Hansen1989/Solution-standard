...
{    
    /// <summary>
    /// A class which represents the DIVISION table in the SolutionDataBase_standard Database.
    /// </summary>
    public partial class DIVISION: IActiveRecord
    {
        ...
        static void SetTestRepo(){
            _testRepo = _testRepo ?? new TestRepository<DIVISION>(new Solution.DataAccess.DataModel.SolutionDataBase_standardDB());
        }
		...
		public void ORMapping(IDataRecord dataRecord)
        {
            ...
            DIV_NAME = readRecord.get_string("DIV_NAME",null);
               
            DIV_TYPE = readRecord.get_int("DIV_TYPE",null);
               
            STOCK_ID = readRecord.get_string("STOCK_ID",null);
               
            DIV_MEMO = readRecord.get_string("DIV_MEMO",null);
        ...
        public string DIV_NAME
        {
            ...
            set
            {
                if(_DIV_NAME!=value || _isLoaded){
                    _DIV_NAME=value;
                    var col=tbl.Columns.SingleOrDefault(x=>x.Name=="DIV_NAME");
                    if(col!=null){
                        if(!_dirtyColumns.Any(x=>x.Name==col.Name) && _isLoaded){
                            _dirtyColumns.Add(col);
                        }
        ...
        public int DIV_TYPE
        {
            ...
            set
            {
                if(_DIV_TYPE!=value || _isLoaded){
                    _DIV_TYPE=value;
                    var col=tbl.Columns.SingleOrDefault(x=>x.Name=="DIV_TYPE");
                    if(col!=null){
                        if(!_dirtyColumns.Any(x=>x.Name==col.Name) && _isLoaded){
                            _dirtyColumns.Add(col);
                        }
        ...
        public string DIV_MEMO
        {
            ...
            set
            {
                if(_DIV_MEMO!=value || _isLoaded){
                    _DIV_MEMO=value;
                    var col=tbl.Columns.SingleOrDefault(x=>x.Name=="DIV_MEMO");
                    if(col!=null){
                        if(!_dirtyColumns.Any(x=>x.Name==col.Name) && _isLoaded){
                            _dirtyColumns.Add(col);
                        }
