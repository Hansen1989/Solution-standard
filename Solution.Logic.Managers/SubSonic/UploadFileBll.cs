...
namespace Solution.Logic.Managers {
	...
	public partial class UploadFileBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_UploadFile";
        private const string const_CacheKey_Date = "Cache_UploadFile_Date";

		#region 单例模式
		//定义单例实体
		private static UploadFileBll _UploadFileBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static UploadFileBll GetInstence() {
			if (_UploadFileBll == null) {
				_UploadFileBll = new UploadFileBll();
			}
			return _UploadFileBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取UploadFile表记录
		/// <summary>
        /// 从IIS缓存中获取UploadFile表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.UploadFile> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.UploadFile>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.UploadFile> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<UploadFile>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = UploadFile.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = UploadFile.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取UploadFile表记录时出现异常", e);
			}
        ...
        public DataAccess.Model.UploadFile GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = UploadFile.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.UploadFile</returns>
        public DataAccess.Model.UploadFile GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = UploadFile.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.UploadFile</returns>
        public DataAccess.Model.UploadFile GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.UploadFile model = null;
                Expression<Func<UploadFile, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "Name" :
                        ...
                        break;
					case "Path" :
						model = list.SingleOrDefault(x => x.Path == (string)value);
                        expression = x => x.Path == (string)value;
                        break;
					case "Ext" :
                        ...
                        break;
					case "Src" :
						model = list.SingleOrDefault(x => x.Src == (string)value);
                        expression = x => x.Src == (string)value;
                        break;
					case "Size" :
						model = list.SingleOrDefault(x => x.Size == (int)value);
                        expression = x => x.Size == (int)value;
                        break;
					case "PicWidth" :
					...
					case "PicHeight" :
                        ...
                        break;
					case "UploadConfig_Id" :
						model = list.SingleOrDefault(x => x.UploadConfig_Id == (int)value);
                        expression = x => x.UploadConfig_Id == (int)value;
                        break;
					case "JoinName" :
                        ...
                        break;
					case "JoinId" :
						model = list.SingleOrDefault(x => x.JoinId == (int)value);
                        expression = x => x.JoinId == (int)value;
                        break;
					case "UserType" :
                        ...
                        break;
					case "UserId" :
						model = list.SingleOrDefault(x => x.UserId == (int)value);
                        expression = x => x.UserId == (int)value;
                        break;
					case "UserName" :
						model = list.SingleOrDefault(x => x.UserName == (string)value);
                        expression = x => x.UserName == (string)value;
                        break;
					case "UserIp" :
						model = list.SingleOrDefault(x => x.UserIp == (string)value);
                        expression = x => x.UserIp == (string)value;
                        break;
					case "AddDate" :
                        ...
                        break;
					case "InfoText" :
						model = list.SingleOrDefault(x => x.InfoText == (string)value);
                        expression = x => x.InfoText == (string)value;
                        break;
					case "RndKey" :
						model = list.SingleOrDefault(x => x.RndKey == (string)value);
                        expression = x => x.RndKey == (string)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = UploadFile.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.UploadFile</returns>
        public DataAccess.Model.UploadFile GetModelForCache(Expression<Func<DataAccess.Model.UploadFile, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.UploadFile model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中UploadFile表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.UploadFile, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将UploadFile记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.UploadFile）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.UploadFile> Transform(IList<UploadFile> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.UploadFile>();
		...
		/// <summary>
		/// 将UploadFile记录实体由普通的实体（DataAccess.Model.UploadFile）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.UploadFile）</param>
		/// <returns>UploadFile</returns>
		public UploadFile Transform(DataAccess.Model.UploadFile model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将UploadFile记录实体由普通实体集（DataAccess.Model.UploadFile）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.UploadFile）</param>
        public IList<UploadFile> Transform(IList<DataAccess.Model.UploadFile> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.UploadFile model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.UploadFile model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "Name" :
                    ...
                    break;
				case "Path" :
					model.Path = (string)value;
                    break;
				case "Ext" :
					model.Ext = (string)value;
                    break;
				case "Src" :
					model.Src = (string)value;
                    break;
				case "Size" :
					model.Size = (int)value;
                    break;
				case "PicWidth" :
				...
				case "PicHeight" :
                    ...
                    break;
				case "UploadConfig_Id" :
					model.UploadConfig_Id = (int)value;
                    break;
				case "JoinName" :
					model.JoinName = (string)value;
                    break;
				case "JoinId" :
					model.JoinId = (int)value;
                    break;
				case "UserType" :
				...
				case "UserName" :
                    ...
                    break;
				case "UserIp" :
					model.UserIp = (string)value;
                    break;
				case "AddDate" :
					model.AddDate = (DateTime)value;
                    break;
				case "InfoText" :
					model.InfoText = (string)value;
                    break;
				case "RndKey" :
					model.RndKey = (string)value;
                    break;
            }
		}
		...
		/// <summary>
		/// 获取UploadFile表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<UploadFile>(wheres);

		}

		/// <summary>
		/// 获取UploadFile表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<UploadFile, bool>> expression) {
            return new Select().From<UploadFile>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.UploadFile> Find(Expression<Func<DataAccess.Model.UploadFile, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return UploadFile.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.UploadFile, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<UploadFile>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取UploadFile表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", UploadFileTable.TableName,  UploadFileTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadFile表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", UploadFileTable.TableName,  UploadFileTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadFile表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<UploadFile, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			UploadFile.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadFile表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "UploadFile", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了UploadFile表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "UploadFile", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对UploadFile表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = UploadFileTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.UploadFile, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<UploadFile>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.UploadFile model, string colName)
		{
		...
		#region 更新UploadFile表指定字段值
		/// <summary>更新UploadFile表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<UploadFile>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了UploadFile表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新UploadFile表指定主键Id的字段值
		/// <summary>更新UploadFile表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了UploadFile表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, UploadFileTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新UploadFile表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了UploadFile表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新UploadFile表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了UploadFile表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //从数据库中查询
                var model = UploadFile.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion

		#region 删除UploadFile表指定UploadConfig_Id的字段值记录
		/// <summary>
		/// 删除UploadFile表指定UploadConfig_Id的字段值记录
		/// </summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="id">记录的主键值</param>
		public void DeleteByUploadConfig_Id(Page page, int id) {
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", UploadFileTable.TableName, UploadFileTable.UploadConfig_Id, id);
            ...
            {
                //删除缓存
                DelCache(x => x.UploadConfig_Id == id);
            }
			
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了UploadFile表UploadConfig_Id值为【" + id + "】的记录！");
		}
		...
		public void DeleteByUploadConfig_Id(Page page, int[] id) {
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", UploadFileTable.TableName, UploadFileTable.UploadConfig_Id, id);
            ...
            if (CommonBll.IsUseCache())
            {
                ...
                {
                    //删除缓存
                    DelCache(x => x.UploadConfig_Id == i);
                }
            }
			
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了UploadFile表UploadConfig_Id值为【" + str + "】的记录！");
		}
		#endregion

		#region 更新UploadFile表指定UploadConfig_Id的字段值
        /// <summary>更新UploadFile表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
	    /// <param name="UploadConfig_Id">字段UploadConfig_Id的值</param>
	    /// <param name="dic">需要更新的字段与值</param>
	    /// <param name="content">更新说明</param>
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_UploadConfig_Id(Page page, int UploadConfig_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了UploadFile表外键UploadConfig_Id值为" + UploadConfig_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, UploadFileTable.UploadConfig_Id, Comparison.Equals, UploadConfig_Id)
            };
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_UploadConfig_Id(Page page, int UploadConfig_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了UploadFile表外键UploadConfig_Id值为" + UploadConfig_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//执行更新
            UpdateValue_For_UploadConfig_Id(page, UploadConfig_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_UploadConfig_Id(Page page, int UploadConfig_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了UploadFile表外键UploadConfig_Id值为" + UploadConfig_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
		...
		public void UpdateUserType(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[UploadFileTable.UserType] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了UploadFile表id为【" + pkValue + "】的记录，更新内容为将UserType字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		
		#region 获取UserName字段值
        /// <summary>
        /// 获取UserName字段值
        /// </summary>
        /// <param name="page">当前页面指针</param>
        ...
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns></returns>
        public string GetUserName(Page page, int pkValue, bool isCache = true)
        {
            //判断是否启用缓存
            if (isCache && CommonBll.IsUseCache())
            {
		...
		#region 使用RndKey来查询，获取一个UploadFile实体对象
        /// <summary>使用Key来查询，获取一个UploadFile实体对象</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns>DataAccess.Model.UploadFile 实体</returns>
        public DataAccess.Model.UploadFile GetModel_ByRndKey(Page page, string key, bool isCache = true)
        {
            ...
            {
                //从缓存中获取实体
                return GetModelForCache(x => x.RndKey == key);
            }
			else
			{
				//从数据库中查询
				return Transform(UploadFile.SingleOrDefault(x => x.RndKey == key));
			}
        }
        #endregion
