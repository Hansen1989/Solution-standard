...
namespace Solution.Logic.Managers {
	...
	public partial class LoginLogBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_LoginLog";
        private const string const_CacheKey_Date = "Cache_LoginLog_Date";
        ...
        /// <returns>DataAccess.Model.LoginLog</returns>
        public DataAccess.Model.LoginLog GetModelForCache(Expression<Func<DataAccess.Model.LoginLog, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.LoginLog model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(LoginLog model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中LoginLog表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.LoginLog, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将LoginLog记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.LoginLog）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.LoginLog</returns>
		public DataAccess.Model.LoginLog Transform(LoginLog model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.LoginLog
            {
                Id = model.Id,
		...
		/// <summary>
		/// 将LoginLog记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.LoginLog）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.LoginLog> Transform(IList<LoginLog> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.LoginLog>();
		...
		/// <summary>
		/// 将LoginLog记录实体由普通的实体（DataAccess.Model.LoginLog）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.LoginLog）</param>
		/// <returns>LoginLog</returns>
		public LoginLog Transform(DataAccess.Model.LoginLog model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将LoginLog记录实体由普通实体集（DataAccess.Model.LoginLog）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.LoginLog）</param>
        public IList<LoginLog> Transform(IList<DataAccess.Model.LoginLog> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.LoginLog model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.LoginLog model, string colName, object value)
		{
		...
		/// <summary>
		/// 获取LoginLog表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<LoginLog>(wheres);

		}

		/// <summary>
		/// 获取LoginLog表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<LoginLog, bool>> expression) {
            return new Select().From<LoginLog>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.LoginLog> Find(Expression<Func<DataAccess.Model.LoginLog, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return LoginLog.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.LoginLog, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<LoginLog>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取LoginLog表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", LoginLogTable.TableName,  LoginLogTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了LoginLog表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", LoginLogTable.TableName,  LoginLogTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了LoginLog表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<LoginLog, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			LoginLog.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了LoginLog表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "LoginLog", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了LoginLog表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "LoginLog", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对LoginLog表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = LoginLogTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.LoginLog, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<LoginLog>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.LoginLog model, string colName)
		{
		...
		#region 更新LoginLog表指定字段值
		/// <summary>更新LoginLog表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<LoginLog>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了LoginLog表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新LoginLog表指定主键Id的字段值
		/// <summary>更新LoginLog表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了LoginLog表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, LoginLogTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新LoginLog表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了LoginLog表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新LoginLog表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了LoginLog表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
