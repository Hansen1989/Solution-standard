...
namespace Solution.Logic.Managers {
	...
	public partial class GROUPAREABll : LogicBase {
		...
		private const string const_CacheKey = "Cache_GROUPAREA";
        private const string const_CacheKey_Date = "Cache_GROUPAREA_Date";

		#region 单例模式
		//定义单例实体
		private static GROUPAREABll _GROUPAREABll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static GROUPAREABll GetInstence() {
			if (_GROUPAREABll == null) {
				_GROUPAREABll = new GROUPAREABll();
			}
			return _GROUPAREABll;
		}
		#endregion
        ...
        /// <returns>DataAccess.Model.GROUPAREA</returns>
        public DataAccess.Model.GROUPAREA GetModelForCache(Expression<Func<DataAccess.Model.GROUPAREA, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.GROUPAREA model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中GROUPAREA表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.GROUPAREA, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将GROUPAREA记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.GROUPAREA）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.GROUPAREA> Transform(IList<GROUPAREA> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.GROUPAREA>();
		...
		/// <summary>
		/// 将GROUPAREA记录实体由普通的实体（DataAccess.Model.GROUPAREA）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.GROUPAREA）</param>
		/// <returns>GROUPAREA</returns>
		public GROUPAREA Transform(DataAccess.Model.GROUPAREA model)
        {
			if (model == null) 
				return null;

            return new GROUPAREA
            {
                Id = model.Id,
                AREA_ID = model.AREA_ID,
                AREA_NAME = model.AREA_NAME,
		...
		/// <summary>
		/// 将GROUPAREA记录实体由普通实体集（DataAccess.Model.GROUPAREA）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.GROUPAREA）</param>
        public IList<GROUPAREA> Transform(IList<DataAccess.Model.GROUPAREA> sourceList)
        {
			//创建List容器
            var list = new List<GROUPAREA>();
		...
		public void SetModelValue(DataAccess.Model.GROUPAREA model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.GROUPAREA model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "AREA_NAME" :
                    ...
                    break;
				case "AREA_ADD" :
					model.AREA_ADD = (string)value;
                    break;
				case "AREA_TEL" :
					model.AREA_TEL = (string)value;
                    break;
				case "AREA_EMAIL" :
					model.AREA_EMAIL = (string)value;
                    break;
				case "AREA_CONTECT" :
					model.AREA_CONTECT = (string)value;
                    break;
				case "AREA_MEMO" :
					model.AREA_MEMO = (string)value;
                    break;
				case "AREA_STATUS" :
					model.AREA_STATUS = ConvertHelper.Ctinyint(value);
                    break;
				case "CRT_DATETIME" :
					model.CRT_DATETIME = (string)value;
                    break;
				case "CRT_USER_ID" :
		...
		/// <summary>
		/// 获取GROUPAREA表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<GROUPAREA>(wheres);

		}

		/// <summary>
		/// 获取GROUPAREA表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<GROUPAREA, bool>> expression) {
            return new Select().From<GROUPAREA>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.GROUPAREA> Find(Expression<Func<DataAccess.Model.GROUPAREA, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.GROUPAREA, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<GROUPAREA>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取GROUPAREA表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", GROUPAREATable.TableName,  GROUPAREATable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了GROUPAREA表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", GROUPAREATable.TableName,  GROUPAREATable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了GROUPAREA表id为【" + str + "】的记录！");
			}
		}
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "GROUPAREA", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了GROUPAREA表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "GROUPAREA", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对GROUPAREA表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = GROUPAREATable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.GROUPAREA, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<GROUPAREA>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.GROUPAREA model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "AREA_ADD" :
					return model.AREA_ADD;
				case "AREA_TEL" :
					return model.AREA_TEL;
				case "AREA_EMAIL" :
					return model.AREA_EMAIL;
				case "AREA_CONTECT" :
					return model.AREA_CONTECT;
				case "AREA_MEMO" :
					return model.AREA_MEMO;
				case "AREA_STATUS" :
					return model.AREA_STATUS;
				case "CRT_DATETIME" :
		...
		#region 更新GROUPAREA表指定字段值
		/// <summary>更新GROUPAREA表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<GROUPAREA>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了GROUPAREA表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新GROUPAREA表指定主键Id的字段值
		/// <summary>更新GROUPAREA表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了GROUPAREA表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, GROUPAREATable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新GROUPAREA表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了GROUPAREA表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新GROUPAREA表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了GROUPAREA表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateAREA_STATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[GROUPAREATable.AREA_STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了GROUPAREA表id为【" + pkValue + "】的记录，更新内容为将AREA_STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public void UpdateSTATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[GROUPAREATable.STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了GROUPAREA表id为【" + pkValue + "】的记录，更新内容为将STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
