...
namespace Solution.Logic.Managers {
	...
	public partial class PositionBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_Position";
        private const string const_CacheKey_Date = "Cache_Position_Date";
		...
		#region 从IIS缓存中获取Position表记录
		/// <summary>
        /// 从IIS缓存中获取Position表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.Position> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.Position>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.Position> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<Position>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = Position.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = Position.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取Position表记录时出现异常", e);
			}
        ...
        public DataAccess.Model.Position GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = Position.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.Position</returns>
        public DataAccess.Model.Position GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = Position.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.Position</returns>
        public DataAccess.Model.Position GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.Position model = null;
                ...
                {
                    //从数据库中读取
                    var tem = Position.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.Position</returns>
        public DataAccess.Model.Position GetModelForCache(Expression<Func<DataAccess.Model.Position, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.Position model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中Position表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.Position, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将Position记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.Position）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.Position> Transform(IList<Position> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.Position>();
		...
		/// <summary>
		/// 将Position记录实体由普通的实体（DataAccess.Model.Position）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.Position）</param>
		/// <returns>Position</returns>
		public Position Transform(DataAccess.Model.Position model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将Position记录实体由普通实体集（DataAccess.Model.Position）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.Position）</param>
        public IList<Position> Transform(IList<DataAccess.Model.Position> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.Position model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.Position model, string colName, object value)
		{
		...
		/// <summary>
		/// 获取Position表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<Position>(wheres);

		}

		/// <summary>
		/// 获取Position表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<Position, bool>> expression) {
            return new Select().From<Position>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.Position> Find(Expression<Func<DataAccess.Model.Position, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return Position.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.Position, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<Position>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取Position表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", PositionTable.TableName,  PositionTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了Position表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PositionTable.TableName,  PositionTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了Position表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<Position, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			Position.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了Position表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "Position", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了Position表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "Position", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对Position表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = PositionTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.Position, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<Position>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.Position model, string colName)
		{
		...
		#region 更新Position表指定字段值
		/// <summary>更新Position表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<Position>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了Position表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新Position表指定主键Id的字段值
		/// <summary>更新Position表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了Position表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PositionTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新Position表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Position表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新Position表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Position表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //从数据库中查询
                var model = Position.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion

		#region 删除Position表指定Branch_Id的字段值记录
		/// <summary>
		/// 删除Position表指定Branch_Id的字段值记录
		/// </summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="id">记录的主键值</param>
		public void DeleteByBranch_Id(Page page, int id) {
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", PositionTable.TableName, PositionTable.Branch_Id, id);
			...
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了Position表Branch_Id值为【" + id + "】的记录！");
		}
		...
		public void DeleteByBranch_Id(Page page, int[] id) {
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PositionTable.TableName, PositionTable.Branch_Id, id);
			...
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了Position表Branch_Id值为【" + str + "】的记录！");
		}
		#endregion

		#region 更新Position表指定Branch_Id的字段值
        /// <summary>更新Position表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了Position表外键Branch_Id值为" + Branch_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, PositionTable.Branch_Id, Comparison.Equals, Branch_Id)
            };
        ...
        {
			content = content != "" ? content : "{0}修改了Position表外键Branch_Id值为" + Branch_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);
        ...
        {
			content = content != "" ? content : "{0}修改了Position表外键Branch_Id值为" + Branch_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
		...
		public void UpdateIsSetBranchPower(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[PositionTable.IsSetBranchPower] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了Position表id为【" + pkValue + "】的记录，更新内容为将IsSetBranchPower字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
