...
namespace Solution.Logic.Managers {
	...
	public partial class OnlineUsersBll : LogicBase {
		...
		#region 实体转换
		/// <summary>
		/// 将OnlineUsers记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.OnlineUsers）
		/// </summary>
		...
		/// <summary>
		/// 将OnlineUsers记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.OnlineUsers）
		/// </summary>
		...
		/// <summary>
		/// 将OnlineUsers记录实体由普通的实体（DataAccess.Model.OnlineUsers）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.OnlineUsers）</param>
		...
		/// <summary>
		/// 将OnlineUsers记录实体由普通实体集（DataAccess.Model.OnlineUsers）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.OnlineUsers）</param>
		...
		/// <summary>
		/// 获取OnlineUsers表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //记录日志
                CommonBll.WriteLog("获取OnlineUsers表记录时出现异常", e);
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了OnlineUsers表id为【" + str + "】的记录！");
			}
		}
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "OnlineUsers", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对OnlineUsers表进行了自动排序操作！");
		...
		/// <summary>更新OnlineUsers表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了OnlineUsers表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新OnlineUsers表指定主键Id的字段值
		/// <summary>更新OnlineUsers表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了OnlineUsers表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, OnlineUsersTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新OnlineUsers表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了OnlineUsers表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新OnlineUsers表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了OnlineUsers表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 使用UserHashKey来查询，获取一个OnlineUsers实体对象
        /// <summary>使用Key来查询，获取一个OnlineUsers实体对象</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns>DataAccess.Model.OnlineUsers 实体</returns>
		...
		#region 获取Manager_LoginName字段值
        /// <summary>
        /// 获取Manager_LoginName字段值
        /// </summary>
        /// <param name="page">当前页面指针</param>
		...
		#region 使用UserKey来查询，获取一个OnlineUsers实体对象
        /// <summary>使用Key来查询，获取一个OnlineUsers实体对象</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否从缓存中读取</param>
		...
		#region 删除OnlineUsers表指定Branch_Id的字段值记录
		/// <summary>
		/// 删除OnlineUsers表指定Branch_Id的字段值记录
		/// </summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="id">记录的主键值</param>
		public void DeleteByBranch_Id(Page page, int id) {
			...
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了OnlineUsers表Branch_Id值为【" + id + "】的记录！");
		}
		...
		public void DeleteByBranch_Id(Page page, int[] id) {
			...
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了OnlineUsers表Branch_Id值为【" + str + "】的记录！");
		}
		#endregion
        ...
        {
			content = content != "" ? content : "{0}修改了OnlineUsers表外键Branch_Id值为" + Branch_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, OnlineUsersTable.Branch_Id, Comparison.Equals, Branch_Id)
            };
        ...
        {
			content = content != "" ? content : "{0}修改了OnlineUsers表外键Branch_Id值为" + Branch_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);
        ...
        {
			content = content != "" ? content : "{0}修改了OnlineUsers表外键Branch_Id值为" + Branch_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
