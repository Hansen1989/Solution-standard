...
namespace Solution.Logic.Managers {
	...
	/// PagePowerSign表逻辑类
	/// </summary>
	public partial class PagePowerSignBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_PagePowerSign";
        private const string const_CacheKey_Date = "Cache_PagePowerSign_Date";
		...
		#region 从IIS缓存中获取PagePowerSign表记录
		/// <summary>
        /// 从IIS缓存中获取PagePowerSign表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.PagePowerSign> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.PagePowerSign>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.PagePowerSign> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<PagePowerSign>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = PagePowerSign.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = PagePowerSign.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取PagePowerSign表记录时出现异常", e);
			}
        ...
        public DataAccess.Model.PagePowerSign GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = PagePowerSign.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.PagePowerSign</returns>
        public DataAccess.Model.PagePowerSign GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = PagePowerSign.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.PagePowerSign</returns>
        public DataAccess.Model.PagePowerSign GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.PagePowerSign model = null;
                Expression<Func<PagePowerSign, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "PagePowerSignPublic_Id" :
						model = list.SingleOrDefault(x => x.PagePowerSignPublic_Id == (int)value);
                        expression = x => x.PagePowerSignPublic_Id == (int)value;
                        break;
					case "CName" :
                ...
                {
                    //从数据库中读取
                    var tem = PagePowerSign.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.PagePowerSign</returns>
        public DataAccess.Model.PagePowerSign GetModelForCache(Expression<Func<DataAccess.Model.PagePowerSign, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.PagePowerSign model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中PagePowerSign表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.PagePowerSign, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将PagePowerSign记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.PagePowerSign）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.PagePowerSign> Transform(IList<PagePowerSign> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.PagePowerSign>();
		...
		/// <summary>
		/// 将PagePowerSign记录实体由普通的实体（DataAccess.Model.PagePowerSign）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.PagePowerSign）</param>
		/// <returns>PagePowerSign</returns>
		public PagePowerSign Transform(DataAccess.Model.PagePowerSign model)
        {
			if (model == null) 
				return null;

            return new PagePowerSign
            {
                Id = model.Id,
                PagePowerSignPublic_Id = model.PagePowerSignPublic_Id,
                CName = model.CName,
		...
		/// <summary>
		/// 将PagePowerSign记录实体由普通实体集（DataAccess.Model.PagePowerSign）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.PagePowerSign）</param>
        public IList<PagePowerSign> Transform(IList<DataAccess.Model.PagePowerSign> sourceList)
        {
			//创建List容器
            var list = new List<PagePowerSign>();
		...
		public void SetModelValue(DataAccess.Model.PagePowerSign model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.PagePowerSign model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "PagePowerSignPublic_Id" :
					model.PagePowerSignPublic_Id = (int)value;
                    break;
				case "CName" :
		...
		#region 获取PagePowerSign表记录总数
        /// <summary>
        /// 获取PagePowerSign表记录总数
        /// </summary>
		...
		/// <summary>
		/// 获取PagePowerSign表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<PagePowerSign, bool>> expression) {
            return new Select().From<PagePowerSign>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.PagePowerSign> Find(Expression<Func<DataAccess.Model.PagePowerSign, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return PagePowerSign.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.PagePowerSign, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<PagePowerSign>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取PagePowerSign表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", PagePowerSignTable.TableName,  PagePowerSignTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PagePowerSignTable.TableName,  PagePowerSignTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<PagePowerSign, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			PagePowerSign.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "PagePowerSign", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了PagePowerSign表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "PagePowerSign", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对PagePowerSign表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = PagePowerSignTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.PagePowerSign, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<PagePowerSign>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.PagePowerSign model, string colName)
		{
			...
			switch (colName)
			{
				case "Id" :
					return model.Id;
				case "PagePowerSignPublic_Id" :
					return model.PagePowerSignPublic_Id;
				case "CName" :
					return model.CName;
				case "EName" :
					return model.EName;
				case "MenuInfo_Id" :
					return model.MenuInfo_Id;
			}
		...
		#region 更新PagePowerSign表指定字段值
		/// <summary>更新PagePowerSign表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<PagePowerSign>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了PagePowerSign表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新PagePowerSign表指定主键Id的字段值
		/// <summary>更新PagePowerSign表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PagePowerSignTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新PagePowerSign表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了PagePowerSign表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新PagePowerSign表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了PagePowerSign表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 删除PagePowerSign表指定PagePowerSignPublic_Id的字段值记录
		/// <summary>
		/// 删除PagePowerSign表指定PagePowerSignPublic_Id的字段值记录
		/// </summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="id">记录的主键值</param>
		public void DeleteByPagePowerSignPublic_Id(Page page, int id) {
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", PagePowerSignTable.TableName, PagePowerSignTable.PagePowerSignPublic_Id, id);
            ...
            {
                //删除缓存
                DelCache(x => x.PagePowerSignPublic_Id == id);
            }
			
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表PagePowerSignPublic_Id值为【" + id + "】的记录！");
		}
		...
		public void DeleteByPagePowerSignPublic_Id(Page page, int[] id) {
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PagePowerSignTable.TableName, PagePowerSignTable.PagePowerSignPublic_Id, id);
            ...
            if (CommonBll.IsUseCache())
            {
                ...
                {
                    //删除缓存
                    DelCache(x => x.PagePowerSignPublic_Id == i);
                }
            }
			
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表PagePowerSignPublic_Id值为【" + str + "】的记录！");
		}
		#endregion

		#region 更新PagePowerSign表指定PagePowerSignPublic_Id的字段值
        /// <summary>更新PagePowerSign表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
	    /// <param name="PagePowerSignPublic_Id">字段PagePowerSignPublic_Id的值</param>
	    /// <param name="dic">需要更新的字段与值</param>
	    /// <param name="content">更新说明</param>
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_PagePowerSignPublic_Id(Page page, int PagePowerSignPublic_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键PagePowerSignPublic_Id值为" + PagePowerSignPublic_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, PagePowerSignTable.PagePowerSignPublic_Id, Comparison.Equals, PagePowerSignPublic_Id)
            };
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_PagePowerSignPublic_Id(Page page, int PagePowerSignPublic_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键PagePowerSignPublic_Id值为" + PagePowerSignPublic_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//执行更新
            UpdateValue_For_PagePowerSignPublic_Id(page, PagePowerSignPublic_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_PagePowerSignPublic_Id(Page page, int PagePowerSignPublic_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键PagePowerSignPublic_Id值为" + PagePowerSignPublic_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
        ...
        public string GetCName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //从数据库中查询
                var model = PagePowerSign.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.CName;
            }
        }
        #endregion
		...
		#region 删除PagePowerSign表指定MenuInfo_Id的字段值记录
		/// <summary>
		...
		public void DeleteByMenuInfo_Id(Page page, int[] id) {
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PagePowerSignTable.TableName, PagePowerSignTable.MenuInfo_Id, id);
			...
			//添加用户操作记录
			UseLogBll.GetInstence().Save(page, "{0}删除了PagePowerSign表MenuInfo_Id值为【" + str + "】的记录！");
		}
		#endregion

		#region 更新PagePowerSign表指定MenuInfo_Id的字段值
        /// <summary>更新PagePowerSign表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
	    /// <param name="MenuInfo_Id">字段MenuInfo_Id的值</param>
	    /// <param name="dic">需要更新的字段与值</param>
	    /// <param name="content">更新说明</param>
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_MenuInfo_Id(Page page, int MenuInfo_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键MenuInfo_Id值为" + MenuInfo_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, PagePowerSignTable.MenuInfo_Id, Comparison.Equals, MenuInfo_Id)
            };
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_MenuInfo_Id(Page page, int MenuInfo_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键MenuInfo_Id值为" + MenuInfo_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//执行更新
            UpdateValue_For_MenuInfo_Id(page, MenuInfo_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_MenuInfo_Id(Page page, int MenuInfo_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了PagePowerSign表外键MenuInfo_Id值为" + MenuInfo_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
