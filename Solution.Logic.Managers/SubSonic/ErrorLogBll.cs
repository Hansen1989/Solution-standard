...
namespace Solution.Logic.Managers {
	...
	public partial class ErrorLogBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_ErrorLog";
        private const string const_CacheKey_Date = "Cache_ErrorLog_Date";
		...
		#region 从IIS缓存中获取ErrorLog表记录
		/// <summary>
        /// 从IIS缓存中获取ErrorLog表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.ErrorLog> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.ErrorLog>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.ErrorLog> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<ErrorLog>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = ErrorLog.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = ErrorLog.Find(exp);
        ...
        public DataAccess.Model.ErrorLog GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = ErrorLog.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.ErrorLog</returns>
        public DataAccess.Model.ErrorLog GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = ErrorLog.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.ErrorLog</returns>
        public DataAccess.Model.ErrorLog GetModelForCache(Expression<Func<DataAccess.Model.ErrorLog, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.ErrorLog model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(ErrorLog model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中ErrorLog表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.ErrorLog, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将ErrorLog记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.ErrorLog）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.ErrorLog</returns>
		public DataAccess.Model.ErrorLog Transform(ErrorLog model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.ErrorLog
            {
                Id = model.Id,
                ErrTime = model.ErrTime,
		...
		/// <summary>
		/// 将ErrorLog记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.ErrorLog）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.ErrorLog> Transform(IList<ErrorLog> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.ErrorLog>();
		...
		/// <summary>
		/// 将ErrorLog记录实体由普通的实体（DataAccess.Model.ErrorLog）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.ErrorLog）</param>
		/// <returns>ErrorLog</returns>
		public ErrorLog Transform(DataAccess.Model.ErrorLog model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将ErrorLog记录实体由普通实体集（DataAccess.Model.ErrorLog）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.ErrorLog）</param>
        public IList<ErrorLog> Transform(IList<DataAccess.Model.ErrorLog> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.ErrorLog model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.ErrorLog model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "ErrTime" :
					model.ErrTime = (DateTime)value;
                    break;
				case "BrowserVersion" :
					model.BrowserVersion = (string)value;
                    break;
				case "BrowserType" :
					model.BrowserType = (string)value;
                    break;
				case "Ip" :
					model.Ip = (string)value;
                    break;
				case "PageUrl" :
					model.PageUrl = (string)value;
                    break;
				case "ErrMessage" :
					model.ErrMessage = (string)value;
                    break;
				case "ErrSource" :
					model.ErrSource = (string)value;
                    break;
				case "StackTrace" :
					model.StackTrace = (string)value;
                    break;
				case "HelpLink" :
					model.HelpLink = (string)value;
                    break;
				case "Type" :
					model.Type = ConvertHelper.Ctinyint(value);
                    break;
            }
		}
		...
		/// <summary>
		/// 获取ErrorLog表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<ErrorLog>(wheres);

		}

		/// <summary>
		/// 获取ErrorLog表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<ErrorLog, bool>> expression) {
            return new Select().From<ErrorLog>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.ErrorLog> Find(Expression<Func<DataAccess.Model.ErrorLog, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return ErrorLog.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.ErrorLog, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<ErrorLog>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取ErrorLog表记录时出现异常", e);
		...
		#region 添加与编辑ErrorLog表记录
		/// <summary>
		/// 添加与编辑ErrorLog记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">ErrorLog表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, ErrorLog model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "ErrorLog记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行ErrorLogBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", ErrorLogTable.TableName,  ErrorLogTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了ErrorLog表id为【" + id + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<ErrorLog, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			ErrorLog.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了ErrorLog表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "ErrorLog", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了ErrorLog表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "ErrorLog", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对ErrorLog表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = ErrorLogTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.ErrorLog, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<ErrorLog>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.ErrorLog model, string colName)
		{
			...
			switch (colName)
			{
				case "Id" :
					return model.Id;
				case "ErrTime" :
					return model.ErrTime;
				case "BrowserVersion" :
					return model.BrowserVersion;
				case "BrowserType" :
					return model.BrowserType;
				case "Ip" :
					return model.Ip;
				case "PageUrl" :
					return model.PageUrl;
				case "ErrMessage" :
					return model.ErrMessage;
				case "ErrSource" :
					return model.ErrSource;
				case "StackTrace" :
					return model.StackTrace;
				case "HelpLink" :
					return model.HelpLink;
				case "Type" :
					return model.Type;
			}
		...
		#region 更新ErrorLog表指定字段值
		/// <summary>更新ErrorLog表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<ErrorLog>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了ErrorLog表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新ErrorLog表指定主键Id的字段值
		/// <summary>更新ErrorLog表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了ErrorLog表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, ErrorLogTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新ErrorLog表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了ErrorLog表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新ErrorLog表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了ErrorLog表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateType(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[ErrorLogTable.Type] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了ErrorLog表id为【" + pkValue + "】的记录，更新内容为将Type字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
