	...
	public partial class InformationBll : LogicBase {
		...
		try
            {
                ...
                switch (conditionColName)
                {
					...
					case "FrontCoverImg" :
                        ...
                        break;
					case "Note" :
						model = list.SingleOrDefault(x => x.Note == (string)value);
                        expression = x => x.Note == (string)value;
                        break;
					case "NewsTime" :
	    ...
	    public void UpdateValue(Page page, int id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationTable.Id, Comparison.Equals, id));
            };
	    ...
	    public void UpdateValue_For_InformationClass_Root_Id(Page page, int InformationClass_Root_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationTable.InformationClass_Root_Id, Comparison.Equals, InformationClass_Root_Id)
            };
	    ...
	    public void UpdateValue_For_InformationClass_Id(Page page, int InformationClass_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            var wheres = new List<ConditionFun.SqlqueryCondition>
            {
                new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationTable.InformationClass_Id, Comparison.Equals, InformationClass_Id)
            };
		...
		public void DelFrontCoverImg(Page page, int pkValue, bool isCache = true) {
			try {
				...
				//设置条件
				var wheres = new List<ConditionFun.SqlqueryCondition>();
				wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationTable.Id, Comparison.Equals, pkValue));
