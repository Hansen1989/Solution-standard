...
namespace Solution.Logic.Managers {
	...
	public partial class SUPPLIERSBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_SUPPLIERS";
        private const string const_CacheKey_Date = "Cache_SUPPLIERS_Date";

		#region 单例模式
		//定义单例实体
		private static SUPPLIERSBll _SUPPLIERSBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static SUPPLIERSBll GetInstence() {
			if (_SUPPLIERSBll == null) {
				_SUPPLIERSBll = new SUPPLIERSBll();
			}
			return _SUPPLIERSBll;
		}
		#endregion
        ...
        /// <returns>DataAccess.Model.SUPPLIERS</returns>
        public DataAccess.Model.SUPPLIERS GetModelForCache(Expression<Func<DataAccess.Model.SUPPLIERS, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.SUPPLIERS model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中SUPPLIERS表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.SUPPLIERS, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将SUPPLIERS记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.SUPPLIERS）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.SUPPLIERS> Transform(IList<SUPPLIERS> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.SUPPLIERS>();
		...
		/// <summary>
		/// 将SUPPLIERS记录实体由普通的实体（DataAccess.Model.SUPPLIERS）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.SUPPLIERS）</param>
		/// <returns>SUPPLIERS</returns>
		public SUPPLIERS Transform(DataAccess.Model.SUPPLIERS model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将SUPPLIERS记录实体由普通实体集（DataAccess.Model.SUPPLIERS）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.SUPPLIERS）</param>
        public IList<SUPPLIERS> Transform(IList<DataAccess.Model.SUPPLIERS> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.SUPPLIERS model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.SUPPLIERS model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "SUP_NAME" :
                    ...
                    break;
				case "SUP_NICKNAME" :
					model.SUP_NICKNAME = (string)value;
                    break;
				case "SUP_TYPE" :
					model.SUP_TYPE = (int)value;
                    break;
				case "SUP_ADD" :
					model.SUP_ADD = (string)value;
                    break;
				case "SUP_TEL" :
					model.SUP_TEL = (string)value;
                    break;
				case "SUP_Email" :
					model.SUP_Email = (string)value;
                    break;
				case "SUP_ZIP" :
					model.SUP_ZIP = (string)value;
                    break;
				case "SUP_Contact" :
					model.SUP_Contact = (string)value;
                    break;
				case "SUP_Mobile" :
					model.SUP_Mobile = (string)value;
                    break;
				case "SUP_CODE_ID" :
					model.SUP_CODE_ID = (string)value;
                    break;
				case "SUP_BankName" :
					model.SUP_BankName = (string)value;
                    break;
				case "SUP_BankID" :
					model.SUP_BankID = (string)value;
                    break;
				case "SUP_PASSWORD" :
					model.SUP_PASSWORD = (string)value;
                    break;
				case "Send_DAY" :
					model.Send_DAY = (int)value;
                    break;
				case "CRT_DATETIME" :
		...
		/// <summary>
		/// 获取SUPPLIERS表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<SUPPLIERS>(wheres);

		}

		/// <summary>
		/// 获取SUPPLIERS表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<SUPPLIERS, bool>> expression) {
            return new Select().From<SUPPLIERS>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.SUPPLIERS> Find(Expression<Func<DataAccess.Model.SUPPLIERS, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.SUPPLIERS, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<SUPPLIERS>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取SUPPLIERS表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", SUPPLIERSTable.TableName,  SUPPLIERSTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了SUPPLIERS表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", SUPPLIERSTable.TableName,  SUPPLIERSTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了SUPPLIERS表id为【" + str + "】的记录！");
			}
		}
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "SUPPLIERS", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了SUPPLIERS表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "SUPPLIERS", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对SUPPLIERS表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = SUPPLIERSTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.SUPPLIERS, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<SUPPLIERS>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.SUPPLIERS model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "SUP_NICKNAME" :
					return model.SUP_NICKNAME;
				case "SUP_TYPE" :
					return model.SUP_TYPE;
				case "SUP_ADD" :
					return model.SUP_ADD;
				case "SUP_TEL" :
					return model.SUP_TEL;
				case "SUP_Email" :
					return model.SUP_Email;
				case "SUP_ZIP" :
					return model.SUP_ZIP;
				case "SUP_Contact" :
					return model.SUP_Contact;
				case "SUP_Mobile" :
					return model.SUP_Mobile;
				case "SUP_CODE_ID" :
					return model.SUP_CODE_ID;
				case "SUP_BankName" :
					return model.SUP_BankName;
				case "SUP_BankID" :
					return model.SUP_BankID;
				case "SUP_PASSWORD" :
					return model.SUP_PASSWORD;
				case "Send_DAY" :
					return model.Send_DAY;
				case "CRT_DATETIME" :
		...
		#region 更新SUPPLIERS表指定字段值
		/// <summary>更新SUPPLIERS表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<SUPPLIERS>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了SUPPLIERS表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新SUPPLIERS表指定主键Id的字段值
		/// <summary>更新SUPPLIERS表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了SUPPLIERS表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, SUPPLIERSTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新SUPPLIERS表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了SUPPLIERS表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新SUPPLIERS表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了SUPPLIERS表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
