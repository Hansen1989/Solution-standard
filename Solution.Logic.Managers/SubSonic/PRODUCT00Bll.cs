...
namespace Solution.Logic.Managers {
	...
	public partial class PRODUCT00Bll : LogicBase {
		...
		private const string const_CacheKey = "Cache_PRODUCT00";
        private const string const_CacheKey_Date = "Cache_PRODUCT00_Date";

		#region 单例模式
		//定义单例实体
		private static PRODUCT00Bll _PRODUCT00Bll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static PRODUCT00Bll GetInstence() {
			if (_PRODUCT00Bll == null) {
				_PRODUCT00Bll = new PRODUCT00Bll();
			}
			return _PRODUCT00Bll;
		}
		#endregion
		...
		#region 从IIS缓存中获取PRODUCT00表记录
		/// <summary>
        /// 从IIS缓存中获取PRODUCT00表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.PRODUCT00> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.PRODUCT00>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.PRODUCT00> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<PRODUCT00>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = PRODUCT00.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = PRODUCT00.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取PRODUCT00表记录时出现异常", e);
			}
        ...
        /// <param name="isCache">是否从缓存中读取</param>
		/// <returns>DataAccess.Model.PRODUCT00</returns>
        public DataAccess.Model.PRODUCT00 GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = PRODUCT00.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.PRODUCT00</returns>
        public DataAccess.Model.PRODUCT00 GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = PRODUCT00.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.PRODUCT00</returns>
        public DataAccess.Model.PRODUCT00 GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.PRODUCT00 model = null;
                Expression<Func<PRODUCT00, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "PROD_NAME1" :
                        ...
                        break;
					case "PROD_NAME2" :
						model = list.SingleOrDefault(x => x.PROD_NAME2 == (string)value);
                        expression = x => x.PROD_NAME2 == (string)value;
                        break;
					case "PROD_KIND" :
					...
					case "DIV_ID" :
                        ...
                        break;
					case "PROD_TYPE" :
						model = list.SingleOrDefault(x => x.PROD_TYPE == (byte)value);
                        expression = x => x.PROD_TYPE == (byte)value;
                        break;
					case "PROD_Source" :
						model = list.SingleOrDefault(x => x.PROD_Source == (int)value);
                        expression = x => x.PROD_Source == (int)value;
                        break;
					case "INV_TYPE" :
						model = list.SingleOrDefault(x => x.INV_TYPE == (int)value);
                        expression = x => x.INV_TYPE == (int)value;
                        break;
					case "STOCK_TYPE" :
						model = list.SingleOrDefault(x => x.STOCK_TYPE == (int)value);
                        expression = x => x.STOCK_TYPE == (int)value;
                        break;
					case "BOM_TYPE" :
						model = list.SingleOrDefault(x => x.BOM_TYPE == (int)value);
                        expression = x => x.BOM_TYPE == (int)value;
                        break;
					case "MarginControl" :
						model = list.SingleOrDefault(x => x.MarginControl == (int)value);
                        expression = x => x.MarginControl == (int)value;
                        break;
					case "PROD_RangTYPE" :
						model = list.SingleOrDefault(x => x.PROD_RangTYPE == (int)value);
                        expression = x => x.PROD_RangTYPE == (int)value;
                        break;
					case "PROD_iRang" :
						model = list.SingleOrDefault(x => x.PROD_iRang == (int)value);
                        expression = x => x.PROD_iRang == (int)value;
                        break;
					case "PROD_SPEC" :
						model = list.SingleOrDefault(x => x.PROD_SPEC == (string)value);
                        expression = x => x.PROD_SPEC == (string)value;
                        break;
					case "PROD_Margin" :
						model = list.SingleOrDefault(x => x.PROD_Margin == (string)value);
                        expression = x => x.PROD_Margin == (string)value;
                        break;
					case "PROD_BARCODE" :
						model = list.SingleOrDefault(x => x.PROD_BARCODE == (string)value);
                        expression = x => x.PROD_BARCODE == (string)value;
                        break;
					case "PROD_UNIT" :
					...
					case "PROD_CONVERT2" :
                        ...
                        break;
					case "Report_UNIT" :
						model = list.SingleOrDefault(x => x.Report_UNIT == (int)value);
                        expression = x => x.Report_UNIT == (int)value;
                        break;
					case "IsBool1" :
					...
					case "IsBool2" :
                        ...
                        break;
					case "IsBool3" :
						model = list.SingleOrDefault(x => x.IsBool3 == (bool)value);
                        expression = x => x.IsBool3 == (bool)value;
                        break;
					case "PROD_MEMO" :
						model = list.SingleOrDefault(x => x.PROD_MEMO == (string)value);
                        expression = x => x.PROD_MEMO == (string)value;
                        break;
					case "CRT_DATETIME" :
                ...
                {
                    //从数据库中读取
                    var tem = PRODUCT00.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.PRODUCT00</returns>
        public DataAccess.Model.PRODUCT00 GetModelForCache(Expression<Func<DataAccess.Model.PRODUCT00, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.PRODUCT00 model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(PRODUCT00 model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中PRODUCT00表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.PRODUCT00, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将PRODUCT00记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.PRODUCT00）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.PRODUCT00</returns>
		public DataAccess.Model.PRODUCT00 Transform(PRODUCT00 model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.PRODUCT00
            {
                Id = model.Id,
                PROD_ID = model.PROD_ID,
                PROD_NAME1 = model.PROD_NAME1,
                PROD_NAME2 = model.PROD_NAME2,
                PROD_KIND = model.PROD_KIND,
                PROD_DEP = model.PROD_DEP,
                PROD_CATE = model.PROD_CATE,
                DIV_ID = model.DIV_ID,
                PROD_TYPE = model.PROD_TYPE,
                PROD_Source = model.PROD_Source,
                INV_TYPE = model.INV_TYPE,
                STOCK_TYPE = model.STOCK_TYPE,
                BOM_TYPE = model.BOM_TYPE,
                MarginControl = model.MarginControl,
                PROD_RangTYPE = model.PROD_RangTYPE,
                PROD_iRang = model.PROD_iRang,
                PROD_SPEC = model.PROD_SPEC,
                PROD_Margin = model.PROD_Margin,
                PROD_BARCODE = model.PROD_BARCODE,
                ...
                Report_UNIT = model.Report_UNIT,
                IsBool1 = model.IsBool1,
                IsBool2 = model.IsBool2,
                IsBool3 = model.IsBool3,
                PROD_MEMO = model.PROD_MEMO,
		...
		/// <summary>
		/// 将PRODUCT00记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.PRODUCT00）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.PRODUCT00> Transform(IList<PRODUCT00> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.PRODUCT00>();
		...
		/// <summary>
		/// 将PRODUCT00记录实体由普通的实体（DataAccess.Model.PRODUCT00）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.PRODUCT00）</param>
		/// <returns>PRODUCT00</returns>
		public PRODUCT00 Transform(DataAccess.Model.PRODUCT00 model)
        {
			if (model == null) 
				return null;

            return new PRODUCT00
            {
                Id = model.Id,
                PROD_ID = model.PROD_ID,
                PROD_NAME1 = model.PROD_NAME1,
		...
		/// <summary>
		/// 将PRODUCT00记录实体由普通实体集（DataAccess.Model.PRODUCT00）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.PRODUCT00）</param>
        public IList<PRODUCT00> Transform(IList<DataAccess.Model.PRODUCT00> sourceList)
        {
			//创建List容器
            var list = new List<PRODUCT00>();
		...
		public void SetModelValue(DataAccess.Model.PRODUCT00 model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.PRODUCT00 model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "PROD_CATE" :
                    ...
                    break;
				case "DIV_ID" :
					model.DIV_ID = (string)value;
                    break;
				case "PROD_TYPE" :
					model.PROD_TYPE = ConvertHelper.Ctinyint(value);
                    break;
				case "PROD_Source" :
					model.PROD_Source = (int)value;
                    break;
				case "INV_TYPE" :
					model.INV_TYPE = (int)value;
                    break;
				case "STOCK_TYPE" :
					model.STOCK_TYPE = (int)value;
                    break;
				case "BOM_TYPE" :
					model.BOM_TYPE = (int)value;
                    break;
				case "MarginControl" :
					model.MarginControl = (int)value;
                    break;
				case "PROD_RangTYPE" :
					model.PROD_RangTYPE = (int)value;
                    break;
				case "PROD_iRang" :
					model.PROD_iRang = (int)value;
                    break;
				case "PROD_SPEC" :
					model.PROD_SPEC = (string)value;
                    break;
				case "PROD_Margin" :
					model.PROD_Margin = (string)value;
                    break;
				case "PROD_BARCODE" :
					model.PROD_BARCODE = (string)value;
                    break;
				case "PROD_UNIT" :
				...
				case "PROD_CONVERT2" :
                    ...
                    break;
				case "Report_UNIT" :
					model.Report_UNIT = (int)value;
                    break;
				case "IsBool1" :
					model.IsBool1 = ConvertHelper.Ctinyint(value);
                    break;
				case "IsBool2" :
					model.IsBool2 = ConvertHelper.Ctinyint(value);
                    break;
				case "IsBool3" :
					model.IsBool3 = (bool)value;
                    break;
				case "PROD_MEMO" :
					model.PROD_MEMO = (string)value;
                    break;
				case "CRT_DATETIME" :
		...
		/// <summary>
		/// 获取PRODUCT00表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<PRODUCT00>(wheres);

		}

		/// <summary>
		/// 获取PRODUCT00表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<PRODUCT00, bool>> expression) {
            return new Select().From<PRODUCT00>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.PRODUCT00> Find(Expression<Func<DataAccess.Model.PRODUCT00, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return PRODUCT00.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.PRODUCT00, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<PRODUCT00>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取PRODUCT00表记录时出现异常", e);
		...
		#region 添加与编辑PRODUCT00表记录
		/// <summary>
		/// 添加与编辑PRODUCT00记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">PRODUCT00表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, PRODUCT00 model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "PRODUCT00记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行PRODUCT00Bll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", PRODUCT00Table.TableName,  PRODUCT00Table.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PRODUCT00表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", PRODUCT00Table.TableName,  PRODUCT00Table.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PRODUCT00表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<PRODUCT00, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			PRODUCT00.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了PRODUCT00表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "PRODUCT00", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了PRODUCT00表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "PRODUCT00", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对PRODUCT00表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = PRODUCT00Table.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.PRODUCT00, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<PRODUCT00>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.PRODUCT00 model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "DIV_ID" :
					return model.DIV_ID;
				case "PROD_TYPE" :
					return model.PROD_TYPE;
				case "PROD_Source" :
					return model.PROD_Source;
				case "INV_TYPE" :
					return model.INV_TYPE;
				case "STOCK_TYPE" :
					return model.STOCK_TYPE;
				case "BOM_TYPE" :
					return model.BOM_TYPE;
				case "MarginControl" :
					return model.MarginControl;
				case "PROD_RangTYPE" :
					return model.PROD_RangTYPE;
				case "PROD_iRang" :
					return model.PROD_iRang;
				case "PROD_SPEC" :
					return model.PROD_SPEC;
				case "PROD_Margin" :
					return model.PROD_Margin;
				case "PROD_BARCODE" :
					return model.PROD_BARCODE;
				case "PROD_UNIT" :
				...
				case "Report_UNIT" :
					return model.Report_UNIT;
				case "IsBool1" :
					return model.IsBool1;
				case "IsBool2" :
					return model.IsBool2;
				case "IsBool3" :
					return model.IsBool3;
				case "PROD_MEMO" :
					return model.PROD_MEMO;
				case "CRT_DATETIME" :
		...
		#region 更新PRODUCT00表指定字段值
		/// <summary>更新PRODUCT00表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<PRODUCT00>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了PRODUCT00表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新PRODUCT00表指定主键Id的字段值
		/// <summary>更新PRODUCT00表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了PRODUCT00表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, PRODUCT00Table.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新PRODUCT00表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了PRODUCT00表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新PRODUCT00表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了PRODUCT00表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdatePROD_TYPE(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[PRODUCT00Table.PROD_TYPE] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了PRODUCT00表id为【" + pkValue + "】的记录，更新内容为将PROD_TYPE字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateIsBool1(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[PRODUCT00Table.IsBool1] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了PRODUCT00表id为【" + pkValue + "】的记录，更新内容为将IsBool1字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateIsBool2(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[PRODUCT00Table.IsBool2] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了PRODUCT00表id为【" + pkValue + "】的记录，更新内容为将IsBool2字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public void UpdateSTATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[PRODUCT00Table.STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了PRODUCT00表id为【" + pkValue + "】的记录，更新内容为将STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
