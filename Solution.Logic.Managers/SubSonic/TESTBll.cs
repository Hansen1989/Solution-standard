...
namespace Solution.Logic.Managers {
	...
	/// TEST表逻辑类
	/// </summary>
	public partial class TESTBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_TEST";
        private const string const_CacheKey_Date = "Cache_TEST_Date";

		#region 单例模式
		//定义单例实体
		private static TESTBll _TESTBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static TESTBll GetInstence() {
			if (_TESTBll == null) {
		...
		#region 从IIS缓存中获取TEST表记录
		/// <summary>
        /// 从IIS缓存中获取TEST表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.TEST> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.TEST>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.TEST> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<TEST>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取TEST表记录时出现异常", e);
			}
        ...
        public DataAccess.Model.TEST GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = TEST.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.TEST</returns>
        public DataAccess.Model.TEST GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = TEST.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.TEST</returns>
        public DataAccess.Model.TEST GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.TEST model = null;
                ...
                switch (conditionColName)
                {
					case "Id" :
                        ...
                        break;
					case "COL1" :
						model = list.SingleOrDefault(x => x.COL1 == (string)value);
                        expression = x => x.COL1 == (string)value;
                        break;
					case "COL2" :
						model = list.SingleOrDefault(x => x.COL2 == (string)value);
                        expression = x => x.COL2 == (string)value;
                        break;
					case "COL3" :
						model = list.SingleOrDefault(x => x.COL3 == (string)value);
                        expression = x => x.COL3 == (string)value;
                        break;
					case "COL4" :
						model = list.SingleOrDefault(x => x.COL4 == (int)value);
                        expression = x => x.COL4 == (int)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = TEST.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.TEST</returns>
        public DataAccess.Model.TEST GetModelForCache(Expression<Func<DataAccess.Model.TEST, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.TEST model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(TEST model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中TEST表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.TEST, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将TEST记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.TEST）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.TEST> Transform(IList<TEST> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.TEST>();
		...
		/// <summary>
		/// 将TEST记录实体由普通的实体（DataAccess.Model.TEST）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.TEST）</param>
		/// <returns>TEST</returns>
		public TEST Transform(DataAccess.Model.TEST model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将TEST记录实体由普通实体集（DataAccess.Model.TEST）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.TEST）</param>
        public IList<TEST> Transform(IList<DataAccess.Model.TEST> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.TEST model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.TEST model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "COL1" :
					model.COL1 = (string)value;
                    break;
				case "COL2" :
					model.COL2 = (string)value;
                    break;
				case "COL3" :
					model.COL3 = (string)value;
                    break;
				case "COL4" :
					model.COL4 = (int)value;
                    break;
            }
		}
		...
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<TEST>(wheres);

		}

		/// <summary>
		/// 获取TEST表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<TEST, bool>> expression) {
            return new Select().From<TEST>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.TEST> Find(Expression<Func<DataAccess.Model.TEST, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.TEST, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<TEST>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取TEST表记录时出现异常", e);
		...
		/// <param name="model">TEST表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, TEST model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "TEST记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行TESTBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", TESTTable.TableName,  TESTTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了TEST表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", TESTTable.TableName,  TESTTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了TEST表id为【" + str + "】的记录！");
			}
		}
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "TEST", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了TEST表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "TEST", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对TEST表进行了自动排序操作！");
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.TEST, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<TEST>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.TEST model, string colName)
		{
		...
		#region 更新TEST表指定字段值
		/// <summary>更新TEST表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了TEST表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新TEST表指定主键Id的字段值
		/// <summary>更新TEST表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了TEST表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, TESTTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新TEST表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了TEST表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新TEST表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了TEST表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
