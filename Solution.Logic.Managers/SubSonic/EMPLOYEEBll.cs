...
namespace Solution.Logic.Managers {
	...
	public partial class EMPLOYEEBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_EMPLOYEE";
        private const string const_CacheKey_Date = "Cache_EMPLOYEE_Date";

		#region 单例模式
		//定义单例实体
		private static EMPLOYEEBll _EMPLOYEEBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static EMPLOYEEBll GetInstence() {
		...
		#region 从IIS缓存中获取EMPLOYEE表记录
		/// <summary>
        /// 从IIS缓存中获取EMPLOYEE表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.EMPLOYEE> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.EMPLOYEE>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.EMPLOYEE> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<EMPLOYEE>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = EMPLOYEE.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = EMPLOYEE.Find(exp);
        ...
        public DataAccess.Model.EMPLOYEE GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = EMPLOYEE.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.EMPLOYEE</returns>
        public DataAccess.Model.EMPLOYEE GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = EMPLOYEE.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.EMPLOYEE</returns>
        public DataAccess.Model.EMPLOYEE GetModelForCache(Expression<Func<DataAccess.Model.EMPLOYEE, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.EMPLOYEE model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(EMPLOYEE model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中EMPLOYEE表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.EMPLOYEE, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将EMPLOYEE记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.EMPLOYEE）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.EMPLOYEE> Transform(IList<EMPLOYEE> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.EMPLOYEE>();
		...
		/// <summary>
		/// 将EMPLOYEE记录实体由普通的实体（DataAccess.Model.EMPLOYEE）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.EMPLOYEE）</param>
		/// <returns>EMPLOYEE</returns>
		public EMPLOYEE Transform(DataAccess.Model.EMPLOYEE model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将EMPLOYEE记录实体由普通实体集（DataAccess.Model.EMPLOYEE）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.EMPLOYEE）</param>
        public IList<EMPLOYEE> Transform(IList<DataAccess.Model.EMPLOYEE> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.EMPLOYEE model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.EMPLOYEE model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "EMP_ID" :
                    ...
                    break;
				case "EMP_NAME" :
					model.EMP_NAME = (string)value;
                    break;
				case "EMP_Birthday" :
					model.EMP_Birthday = (DateTime)value;
                    break;
				case "EMP_ADD" :
					model.EMP_ADD = (string)value;
                    break;
				case "EMP_TEL" :
					model.EMP_TEL = (string)value;
                    break;
				case "EMP_ZIP" :
					model.EMP_ZIP = (string)value;
                    break;
				case "EMP_EMAIL" :
					model.EMP_EMAIL = (string)value;
                    break;
				case "EMP_MOBILE" :
					model.EMP_MOBILE = (string)value;
                    break;
				case "EMP_MEMO" :
					model.EMP_MEMO = (string)value;
                    break;
				case "EMP_ENABLE" :
					model.EMP_ENABLE = (int)value;
                    break;
				case "EMP_SEX" :
					model.EMP_SEX = (int)value;
                    break;
				case "EMP_CodeID" :
					model.EMP_CodeID = (string)value;
                    break;
				case "EMP_LEVEL" :
					model.EMP_LEVEL = (int)value;
                    break;
				case "EMP_PASSWORD" :
					model.EMP_PASSWORD = (string)value;
                    break;
				case "EMP_BDATE" :
					model.EMP_BDATE = (DateTime)value;
                    break;
				case "EMP_EDATE" :
					model.EMP_EDATE = (DateTime)value;
                    break;
				case "EMP_WAGE" :
					model.EMP_WAGE = (decimal)value;
                    break;
				case "EMP_Education" :
					model.EMP_Education = (int)value;
                    break;
				case "CRT_DATETIME" :
		...
		/// <summary>
		/// 获取EMPLOYEE表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<EMPLOYEE>(wheres);

		}

		/// <summary>
		/// 获取EMPLOYEE表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<EMPLOYEE, bool>> expression) {
            return new Select().From<EMPLOYEE>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.EMPLOYEE> Find(Expression<Func<DataAccess.Model.EMPLOYEE, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return EMPLOYEE.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.EMPLOYEE, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<EMPLOYEE>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取EMPLOYEE表记录时出现异常", e);
		...
		#region 添加与编辑EMPLOYEE表记录
		/// <summary>
		/// 添加与编辑EMPLOYEE记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">EMPLOYEE表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, EMPLOYEE model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "EMPLOYEE记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行EMPLOYEEBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", EMPLOYEETable.TableName,  EMPLOYEETable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了EMPLOYEE表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", EMPLOYEETable.TableName,  EMPLOYEETable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了EMPLOYEE表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<EMPLOYEE, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			EMPLOYEE.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了EMPLOYEE表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "EMPLOYEE", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了EMPLOYEE表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "EMPLOYEE", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对EMPLOYEE表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = EMPLOYEETable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.EMPLOYEE, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<EMPLOYEE>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.EMPLOYEE model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "EMP_Birthday" :
					return model.EMP_Birthday;
				case "EMP_ADD" :
					return model.EMP_ADD;
				case "EMP_TEL" :
					return model.EMP_TEL;
				case "EMP_ZIP" :
					return model.EMP_ZIP;
				case "EMP_EMAIL" :
					return model.EMP_EMAIL;
				case "EMP_MOBILE" :
					return model.EMP_MOBILE;
				case "EMP_MEMO" :
					return model.EMP_MEMO;
				case "EMP_ENABLE" :
					return model.EMP_ENABLE;
				case "EMP_SEX" :
					return model.EMP_SEX;
				case "EMP_CodeID" :
					return model.EMP_CodeID;
				case "EMP_LEVEL" :
					return model.EMP_LEVEL;
				case "EMP_PASSWORD" :
					return model.EMP_PASSWORD;
				case "EMP_BDATE" :
					return model.EMP_BDATE;
				case "EMP_EDATE" :
					return model.EMP_EDATE;
				case "EMP_WAGE" :
					return model.EMP_WAGE;
				case "EMP_Education" :
					return model.EMP_Education;
				case "CRT_DATETIME" :
		...
		#region 更新EMPLOYEE表指定字段值
		/// <summary>更新EMPLOYEE表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<EMPLOYEE>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了EMPLOYEE表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新EMPLOYEE表指定主键Id的字段值
		/// <summary>更新EMPLOYEE表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了EMPLOYEE表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, EMPLOYEETable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新EMPLOYEE表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了EMPLOYEE表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新EMPLOYEE表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了EMPLOYEE表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		public void UpdateSTATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[EMPLOYEETable.STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了EMPLOYEE表id为【" + pkValue + "】的记录，更新内容为将STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
