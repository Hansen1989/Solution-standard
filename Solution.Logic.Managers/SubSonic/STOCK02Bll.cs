...
namespace Solution.Logic.Managers {
	...
	/// STOCK02表逻辑类
	/// </summary>
	public partial class STOCK02Bll : LogicBase {
		...
		private const string const_CacheKey = "Cache_STOCK02";
        private const string const_CacheKey_Date = "Cache_STOCK02_Date";

		#region 单例模式
		//定义单例实体
		private static STOCK02Bll _STOCK02Bll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static STOCK02Bll GetInstence() {
			if (_STOCK02Bll == null) {
				_STOCK02Bll = new STOCK02Bll();
			}
			return _STOCK02Bll;
		}
		#endregion
		...
		#region 从IIS缓存中获取STOCK02表记录
		/// <summary>
        /// 从IIS缓存中获取STOCK02表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.STOCK02> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.STOCK02>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.STOCK02> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<STOCK02>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取STOCK02表记录时出现异常", e);
			}
        ...
        /// <param name="isCache">是否从缓存中读取</param>
		/// <returns>DataAccess.Model.STOCK02</returns>
        public DataAccess.Model.STOCK02 GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = STOCK02.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.STOCK02</returns>
        public DataAccess.Model.STOCK02 GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = STOCK02.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.STOCK02</returns>
        public DataAccess.Model.STOCK02 GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.STOCK02 model = null;
                Expression<Func<STOCK02, bool>> expression = null;
                ...
                {
                    //从数据库中读取
                    var tem = STOCK02.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.STOCK02</returns>
        public DataAccess.Model.STOCK02 GetModelForCache(Expression<Func<DataAccess.Model.STOCK02, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.STOCK02 model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(STOCK02 model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中STOCK02表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.STOCK02, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将STOCK02记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.STOCK02）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.STOCK02</returns>
		public DataAccess.Model.STOCK02 Transform(STOCK02 model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.STOCK02
            {
                Id = model.Id,
		...
		/// <summary>
		/// 将STOCK02记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.STOCK02）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.STOCK02> Transform(IList<STOCK02> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.STOCK02>();
		...
		/// <summary>
		/// 将STOCK02记录实体由普通的实体（DataAccess.Model.STOCK02）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.STOCK02）</param>
		/// <returns>STOCK02</returns>
		public STOCK02 Transform(DataAccess.Model.STOCK02 model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将STOCK02记录实体由普通实体集（DataAccess.Model.STOCK02）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.STOCK02）</param>
        public IList<STOCK02> Transform(IList<DataAccess.Model.STOCK02> sourceList)
        {
			//创建List容器
            var list = new List<STOCK02>();
		...
		public void SetModelValue(DataAccess.Model.STOCK02 model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.STOCK02 model, string colName, object value)
		{
		...
		/// <summary>
		/// 获取STOCK02表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<STOCK02>(wheres);

		}

		/// <summary>
		/// 获取STOCK02表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<STOCK02, bool>> expression) {
            return new Select().From<STOCK02>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.STOCK02> Find(Expression<Func<DataAccess.Model.STOCK02, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return STOCK02.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.STOCK02, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<STOCK02>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取STOCK02表记录时出现异常", e);
		...
		#region 添加与编辑STOCK02表记录
		/// <summary>
		/// 添加与编辑STOCK02记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">STOCK02表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, STOCK02 model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "STOCK02记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行STOCK02Bll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", STOCK02Table.TableName,  STOCK02Table.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了STOCK02表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", STOCK02Table.TableName,  STOCK02Table.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了STOCK02表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<STOCK02, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			STOCK02.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了STOCK02表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "STOCK02", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了STOCK02表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "STOCK02", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对STOCK02表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = STOCK02Table.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.STOCK02, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<STOCK02>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.STOCK02 model, string colName)
		{
		...
		#region 更新STOCK02表指定字段值
		/// <summary>更新STOCK02表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<STOCK02>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了STOCK02表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新STOCK02表指定主键Id的字段值
		/// <summary>更新STOCK02表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了STOCK02表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, STOCK02Table.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新STOCK02表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了STOCK02表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新STOCK02表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了STOCK02表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		public void UpdateUSABLE(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[STOCK02Table.USABLE] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了STOCK02表id为【" + pkValue + "】的记录，更新内容为将USABLE字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
