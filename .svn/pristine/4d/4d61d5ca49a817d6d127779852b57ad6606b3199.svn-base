...
namespace Solution.Logic.Managers {
	...
	public partial class UploadTypeBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_UploadType";
        private const string const_CacheKey_Date = "Cache_UploadType_Date";

		#region 单例模式
		//定义单例实体
		private static UploadTypeBll _UploadTypeBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static UploadTypeBll GetInstence() {
			if (_UploadTypeBll == null) {
				_UploadTypeBll = new UploadTypeBll();
			}
			return _UploadTypeBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取UploadType表记录
		/// <summary>
        /// 从IIS缓存中获取UploadType表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.UploadType> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.UploadType>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.UploadType> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<UploadType>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = UploadType.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = UploadType.Find(exp);
        ...
        /// <returns>DataAccess.Model.UploadType</returns>
        public DataAccess.Model.UploadType GetModelForCache(Expression<Func<DataAccess.Model.UploadType, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.UploadType model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(UploadType model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中UploadType表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.UploadType, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将UploadType记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.UploadType）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.UploadType</returns>
		public DataAccess.Model.UploadType Transform(UploadType model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.UploadType
            {
                Id = model.Id,
		...
		/// <summary>
		/// 将UploadType记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.UploadType）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.UploadType> Transform(IList<UploadType> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.UploadType>();
		...
		/// <summary>
		/// 将UploadType记录实体由普通的实体（DataAccess.Model.UploadType）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.UploadType）</param>
		/// <returns>UploadType</returns>
		public UploadType Transform(DataAccess.Model.UploadType model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将UploadType记录实体由普通实体集（DataAccess.Model.UploadType）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.UploadType）</param>
        public IList<UploadType> Transform(IList<DataAccess.Model.UploadType> sourceList)
        {
			//创建List容器
            var list = new List<UploadType>();
		...
		public void SetModelValue(DataAccess.Model.UploadType model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.UploadType model, string colName, object value)
		{
		...
		/// <summary>
		/// 获取UploadType表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<UploadType>(wheres);

		}

		/// <summary>
		/// 获取UploadType表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<UploadType, bool>> expression) {
            return new Select().From<UploadType>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.UploadType> Find(Expression<Func<DataAccess.Model.UploadType, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return UploadType.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.UploadType, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<UploadType>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取UploadType表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", UploadTypeTable.TableName,  UploadTypeTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadType表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", UploadTypeTable.TableName,  UploadTypeTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadType表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<UploadType, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			UploadType.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了UploadType表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "UploadType", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了UploadType表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "UploadType", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对UploadType表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = UploadTypeTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.UploadType, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<UploadType>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.UploadType model, string colName)
		{
		...
		#region 更新UploadType表指定字段值
		/// <summary>更新UploadType表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<UploadType>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了UploadType表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新UploadType表指定主键Id的字段值
		/// <summary>更新UploadType表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了UploadType表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, UploadTypeTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新UploadType表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了UploadType表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新UploadType表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了UploadType表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 使用TypeKey来查询，获取一个UploadType实体对象
        /// <summary>使用Key来查询，获取一个UploadType实体对象</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns>DataAccess.Model.UploadType 实体</returns>
        public DataAccess.Model.UploadType GetModel_ByTypeKey(Page page, string key, bool isCache = true)
        {
            ...
            {
                //从缓存中获取实体
                return GetModelForCache(x => x.TypeKey == key);
            }
			else
			{
				//从数据库中查询
				return Transform(UploadType.SingleOrDefault(x => x.TypeKey == key));
			}
        }
        #endregion
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //从数据库中查询
                var model = UploadType.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion
		...
		public void UpdateIsSys(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[UploadTypeTable.IsSys] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了UploadType表id为【" + pkValue + "】的记录，更新内容为将IsSys字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
