...
namespace Solution.Logic.Managers {
	...
	public partial class WebConfigBll : LogicBase {
		...
		#region 从IIS缓存中获取WebConfig表记录
		/// <summary>
        /// 从IIS缓存中获取WebConfig表记录
        /// </summary>
		...
		#region 实体转换
		/// <summary>
		/// 将WebConfig记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.WebConfig）
		/// </summary>
		...
		/// <summary>
		/// 将WebConfig记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.WebConfig）
		/// </summary>
		...
		/// <summary>
		/// 将WebConfig记录实体由普通的实体（DataAccess.Model.WebConfig）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.WebConfig）</param>
		...
		/// <summary>
		/// 将WebConfig记录实体由普通实体集（DataAccess.Model.WebConfig）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.WebConfig）</param>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
            ...
            {
                //记录日志
                CommonBll.WriteLog("获取WebConfig表记录时出现异常", e);
		...
		#region 添加与编辑WebConfig表记录
		/// <summary>
		/// 添加与编辑WebConfig记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">WebConfig表实体</param>
        ...
        public void Save(Page page, WebConfig model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "WebConfig记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行WebConfigBll.Save()函数出错！";
		...
		public override void Delete(Page page, int id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了WebConfig表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了WebConfig表id为【" + str + "】的记录！");
			}
		}
        ...
        public void Delete(Page page, Expression<Func<WebConfig, bool>> expression, bool isAddUseLog = true)
        {
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了WebConfig表记录！");
			}
        }
		#endregion
		...
		public override bool UpdateAutoSort(Page page, string strWhere = "", bool isExistsMoreLv = false, int pid = 0, string fieldName = "Sort", string fieldParentId = "ParentId")
	    {
			...
			if (CommonBll.AutoSort("Id", "WebConfig", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对WebConfig表进行了自动排序操作！");
		...
		/// <summary>更新WebConfig表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了WebConfig表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新WebConfig表指定主键Id的字段值
		/// <summary>更新WebConfig表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了WebConfig表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, WebConfigTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新WebConfig表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了WebConfig表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新WebConfig表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了WebConfig表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
