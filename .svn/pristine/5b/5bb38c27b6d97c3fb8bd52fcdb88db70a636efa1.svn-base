...
namespace Solution.Logic.Managers {
	...
	public partial class DIVISIONBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_DIVISION";
        private const string const_CacheKey_Date = "Cache_DIVISION_Date";

		#region 单例模式
		//定义单例实体
		private static DIVISIONBll _DIVISIONBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static DIVISIONBll GetInstence() {
			if (_DIVISIONBll == null) {
				_DIVISIONBll = new DIVISIONBll();
			}
			return _DIVISIONBll;
		}
		#endregion
        ...
        /// <returns>DataAccess.Model.DIVISION</returns>
        public DataAccess.Model.DIVISION GetModelForCache(Expression<Func<DataAccess.Model.DIVISION, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.DIVISION model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中DIVISION表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.DIVISION, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将DIVISION记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.DIVISION）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.DIVISION> Transform(IList<DIVISION> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.DIVISION>();
		...
		/// <summary>
		/// 将DIVISION记录实体由普通的实体（DataAccess.Model.DIVISION）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.DIVISION）</param>
		/// <returns>DIVISION</returns>
		public DIVISION Transform(DataAccess.Model.DIVISION model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将DIVISION记录实体由普通实体集（DataAccess.Model.DIVISION）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.DIVISION）</param>
        public IList<DIVISION> Transform(IList<DataAccess.Model.DIVISION> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.DIVISION model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.DIVISION model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "DIV_ID" :
                    ...
                    break;
				case "DIV_NAME" :
					model.DIV_NAME = (string)value;
                    break;
				case "DIV_TYPE" :
					model.DIV_TYPE = (int)value;
                    break;
				case "STOCK_ID" :
					model.STOCK_ID = (string)value;
                    break;
				case "DIV_MEMO" :
					model.DIV_MEMO = (string)value;
                    break;
				case "CRT_DATETIME" :
		...
		/// <summary>
		/// 获取DIVISION表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<DIVISION>(wheres);

		}

		/// <summary>
		/// 获取DIVISION表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<DIVISION, bool>> expression) {
            return new Select().From<DIVISION>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.DIVISION> Find(Expression<Func<DataAccess.Model.DIVISION, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.DIVISION, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<DIVISION>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取DIVISION表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", DIVISIONTable.TableName,  DIVISIONTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了DIVISION表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", DIVISIONTable.TableName,  DIVISIONTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了DIVISION表id为【" + str + "】的记录！");
			}
		}
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "DIVISION", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了DIVISION表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "DIVISION", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对DIVISION表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = DIVISIONTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.DIVISION, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<DIVISION>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.DIVISION model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "DIV_TYPE" :
					return model.DIV_TYPE;
				case "STOCK_ID" :
					return model.STOCK_ID;
				case "DIV_MEMO" :
					return model.DIV_MEMO;
				case "CRT_DATETIME" :
		...
		#region 更新DIVISION表指定字段值
		/// <summary>更新DIVISION表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<DIVISION>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了DIVISION表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新DIVISION表指定主键Id的字段值
		/// <summary>更新DIVISION表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了DIVISION表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, DIVISIONTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新DIVISION表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了DIVISION表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新DIVISION表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了DIVISION表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		public void UpdateSTATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			...
			setValue[DIVISIONTable.STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了DIVISION表id为【" + pkValue + "】的记录，更新内容为将STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
