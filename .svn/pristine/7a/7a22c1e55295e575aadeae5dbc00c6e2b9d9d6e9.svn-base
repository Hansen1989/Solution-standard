...
namespace Solution.Logic.Managers {
	...
	/// COL_ORDER02表逻辑类
	/// </summary>
	public partial class COL_ORDER02Bll : LogicBase {
		...
		private const string const_CacheKey = "Cache_COL_ORDER02";
        private const string const_CacheKey_Date = "Cache_COL_ORDER02_Date";

		#region 单例模式
		//定义单例实体
		private static COL_ORDER02Bll _COL_ORDER02Bll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static COL_ORDER02Bll GetInstence() {
			if (_COL_ORDER02Bll == null) {
				_COL_ORDER02Bll = new COL_ORDER02Bll();
			}
			return _COL_ORDER02Bll;
		}
		#endregion
		...
		#region 从IIS缓存中获取COL_ORDER02表记录
		/// <summary>
        /// 从IIS缓存中获取COL_ORDER02表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.COL_ORDER02> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.COL_ORDER02>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.COL_ORDER02> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<COL_ORDER02>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = COL_ORDER02.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = COL_ORDER02.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取COL_ORDER02表记录时出现异常", e);
			}
        ...
        /// <param name="isCache">是否从缓存中读取</param>
		/// <returns>DataAccess.Model.COL_ORDER02</returns>
        public DataAccess.Model.COL_ORDER02 GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = COL_ORDER02.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.COL_ORDER02</returns>
        public DataAccess.Model.COL_ORDER02 GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = COL_ORDER02.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.COL_ORDER02</returns>
        public DataAccess.Model.COL_ORDER02 GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.COL_ORDER02 model = null;
                Expression<Func<COL_ORDER02, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "SNo" :
                        ...
                        break;
					case "Import_Shop" :
						model = list.SingleOrDefault(x => x.Import_Shop == (string)value);
                        expression = x => x.Import_Shop == (string)value;
                        break;
					case "PROD_ID" :
                        ...
                        break;
					case "OUT_QUAN" :
						model = list.SingleOrDefault(x => x.OUT_QUAN == (double)value);
                        expression = x => x.OUT_QUAN == (double)value;
                        break;
					case "SUP_QUAN" :
						model = list.SingleOrDefault(x => x.SUP_QUAN == (double)value);
                        expression = x => x.SUP_QUAN == (double)value;
                        break;
					case "STD_QUAN" :
						model = list.SingleOrDefault(x => x.STD_QUAN == (double)value);
                        expression = x => x.STD_QUAN == (double)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = COL_ORDER02.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.COL_ORDER02</returns>
        public DataAccess.Model.COL_ORDER02 GetModelForCache(Expression<Func<DataAccess.Model.COL_ORDER02, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.COL_ORDER02 model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(COL_ORDER02 model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中COL_ORDER02表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.COL_ORDER02, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将COL_ORDER02记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.COL_ORDER02）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.COL_ORDER02</returns>
		public DataAccess.Model.COL_ORDER02 Transform(COL_ORDER02 model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.COL_ORDER02
            {
                Id = model.Id,
                SHOP_ID = model.SHOP_ID,
                COL_ID = model.COL_ID,
                SNo = model.SNo,
                Import_Shop = model.Import_Shop,
                PROD_ID = model.PROD_ID,
                OUT_QUAN = model.OUT_QUAN,
                SUP_QUAN = model.SUP_QUAN,
                STD_QUAN = model.STD_QUAN,
            };
        }

		/// <summary>
		/// 将COL_ORDER02记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.COL_ORDER02）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.COL_ORDER02> Transform(IList<COL_ORDER02> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.COL_ORDER02>();
		...
		/// <summary>
		/// 将COL_ORDER02记录实体由普通的实体（DataAccess.Model.COL_ORDER02）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.COL_ORDER02）</param>
		/// <returns>COL_ORDER02</returns>
		public COL_ORDER02 Transform(DataAccess.Model.COL_ORDER02 model)
        {
			if (model == null) 
				return null;

            return new COL_ORDER02
            {
                Id = model.Id,
                SHOP_ID = model.SHOP_ID,
                COL_ID = model.COL_ID,
		...
		/// <summary>
		/// 将COL_ORDER02记录实体由普通实体集（DataAccess.Model.COL_ORDER02）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.COL_ORDER02）</param>
        public IList<COL_ORDER02> Transform(IList<DataAccess.Model.COL_ORDER02> sourceList)
        {
			//创建List容器
            var list = new List<COL_ORDER02>();
		...
		public void SetModelValue(DataAccess.Model.COL_ORDER02 model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.COL_ORDER02 model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "SNo" :
                    ...
                    break;
				case "Import_Shop" :
					model.Import_Shop = (string)value;
                    break;
				case "PROD_ID" :
					model.PROD_ID = (string)value;
                    break;
				case "OUT_QUAN" :
					model.OUT_QUAN = (double)value;
                    break;
				case "SUP_QUAN" :
					model.SUP_QUAN = (double)value;
                    break;
				case "STD_QUAN" :
					model.STD_QUAN = (double)value;
                    break;
            }
		}
		...
		/// <summary>
		/// 获取COL_ORDER02表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<COL_ORDER02>(wheres);

		}

		/// <summary>
		/// 获取COL_ORDER02表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<COL_ORDER02, bool>> expression) {
            return new Select().From<COL_ORDER02>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.COL_ORDER02> Find(Expression<Func<DataAccess.Model.COL_ORDER02, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return COL_ORDER02.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.COL_ORDER02, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<COL_ORDER02>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取COL_ORDER02表记录时出现异常", e);
		...
		#region 添加与编辑COL_ORDER02表记录
		/// <summary>
		/// 添加与编辑COL_ORDER02记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">COL_ORDER02表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, COL_ORDER02 model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "COL_ORDER02记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行COL_ORDER02Bll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", COL_ORDER02Table.TableName,  COL_ORDER02Table.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了COL_ORDER02表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", COL_ORDER02Table.TableName,  COL_ORDER02Table.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了COL_ORDER02表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<COL_ORDER02, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			COL_ORDER02.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了COL_ORDER02表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "COL_ORDER02", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了COL_ORDER02表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "COL_ORDER02", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对COL_ORDER02表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = COL_ORDER02Table.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.COL_ORDER02, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<COL_ORDER02>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.COL_ORDER02 model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "SNo" :
					return model.SNo;
				case "Import_Shop" :
					return model.Import_Shop;
				case "PROD_ID" :
					return model.PROD_ID;
				case "OUT_QUAN" :
					return model.OUT_QUAN;
				case "SUP_QUAN" :
					return model.SUP_QUAN;
				case "STD_QUAN" :
		...
		#region 更新COL_ORDER02表指定字段值
		/// <summary>更新COL_ORDER02表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<COL_ORDER02>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了COL_ORDER02表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新COL_ORDER02表指定主键Id的字段值
		/// <summary>更新COL_ORDER02表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了COL_ORDER02表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, COL_ORDER02Table.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新COL_ORDER02表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了COL_ORDER02表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新COL_ORDER02表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了COL_ORDER02表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
