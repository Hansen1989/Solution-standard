...
namespace Solution.Logic.Managers {
	...
	public partial class InformationClassBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_InformationClass";
        private const string const_CacheKey_Date = "Cache_InformationClass_Date";

		#region 单例模式
		//定义单例实体
		private static InformationClassBll _InformationClassBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static InformationClassBll GetInstence() {
			if (_InformationClassBll == null) {
				_InformationClassBll = new InformationClassBll();
			}
			return _InformationClassBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取InformationClass表记录
		/// <summary>
        /// 从IIS缓存中获取InformationClass表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.InformationClass> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.InformationClass>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.InformationClass> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<InformationClass>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = InformationClass.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = InformationClass.Find(exp);
            ...
            {
                //记录日志
                CommonBll.WriteLog("从IIS缓存中获取InformationClass表记录时出现异常", e);
			}
        ...
        public DataAccess.Model.InformationClass GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = InformationClass.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.InformationClass</returns>
        public DataAccess.Model.InformationClass GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = InformationClass.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.InformationClass</returns>
        public DataAccess.Model.InformationClass GetModelForCache(string conditionColName, object value)
        {
		try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.InformationClass model = null;
                Expression<Func<InformationClass, bool>> expression = null;
                ...
                switch (conditionColName)
                {
					...
					case "IsSys" :
                        ...
                        break;
					case "BigPicImg" :
						model = list.SingleOrDefault(x => x.BigPicImg == (string)value);
                        expression = x => x.BigPicImg == (string)value;
                        break;
					case "SmallPicImg" :
						model = list.SingleOrDefault(x => x.SmallPicImg == (string)value);
                        expression = x => x.SmallPicImg == (string)value;
                        break;
					case "IsShow" :
						model = list.SingleOrDefault(x => x.IsShow == (byte)value);
                        expression = x => x.IsShow == (byte)value;
                        break;
					case "IsPage" :
                        ...
                        break;
					case "RootId" :
						model = list.SingleOrDefault(x => x.RootId == (int)value);
                        expression = x => x.RootId == (int)value;
                        break;
					case "ParentId" :
                ...
                {
                    //从数据库中读取
                    var tem = InformationClass.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.InformationClass</returns>
        public DataAccess.Model.InformationClass GetModelForCache(Expression<Func<DataAccess.Model.InformationClass, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.InformationClass model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中InformationClass表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.InformationClass, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
        ...
        {			
            ...
            return new DataAccess.Model.InformationClass
            {
                ...
                IsSys = model.IsSys,
                BigPicImg = model.BigPicImg,
                SmallPicImg = model.SmallPicImg,
                IsShow = model.IsShow,
                IsPage = model.IsPage,
                RootId = model.RootId,
		...
		/// <summary>
		/// 将InformationClass记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.InformationClass）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.InformationClass> Transform(IList<InformationClass> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.InformationClass>();
		...
		/// <summary>
		/// 将InformationClass记录实体由普通的实体（DataAccess.Model.InformationClass）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.InformationClass）</param>
		/// <returns>InformationClass</returns>
		public InformationClass Transform(DataAccess.Model.InformationClass model)
        {
			if (model == null) 
				return null;

            return new InformationClass
            {
                Id = model.Id,
                Name = model.Name,
                Content = model.Content,
		...
		/// <summary>
		/// 将InformationClass记录实体由普通实体集（DataAccess.Model.InformationClass）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.InformationClass）</param>
        public IList<InformationClass> Transform(IList<DataAccess.Model.InformationClass> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.InformationClass model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.InformationClass model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "Content" :
                    ...
                    break;
				case "IsSys" :
					model.IsSys = ConvertHelper.Ctinyint(value);
                    break;
				case "BigPicImg" :
					model.BigPicImg = (string)value;
                    break;
				case "SmallPicImg" :
					model.SmallPicImg = (string)value;
                    break;
				case "IsShow" :
					model.IsShow = ConvertHelper.Ctinyint(value);
                    break;
				case "IsPage" :
		...
		#region 获取InformationClass表记录总数
        /// <summary>
        /// 获取InformationClass表记录总数
        /// </summary>
		...
		/// <summary>
		/// 获取InformationClass表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<InformationClass, bool>> expression) {
            return new Select().From<InformationClass>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.InformationClass> Find(Expression<Func<DataAccess.Model.InformationClass, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return InformationClass.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.InformationClass, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		#region 获取InformationClass表记录
		/// <summary>
		/// 获取InformationClass表记录
		/// </summary>
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<InformationClass>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取InformationClass表记录时出现异常", e);
		...
		#region 添加与编辑InformationClass表记录
		/// <summary>
		...
		#region 删除InformationClass表记录
		/// <summary>
		/// 删除InformationClass表记录
		/// </summary>
		/// <param name="page">当前页面指针</param>
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", InformationClassTable.TableName,  InformationClassTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了InformationClass表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", InformationClassTable.TableName,  InformationClassTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了InformationClass表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<InformationClass, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			InformationClass.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了InformationClass表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "InformationClass", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了InformationClass表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "InformationClass", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对InformationClass表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = InformationClassTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.InformationClass, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<InformationClass>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.InformationClass model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "IsSys" :
					return model.IsSys;
				case "BigPicImg" :
					return model.BigPicImg;
				case "SmallPicImg" :
					return model.SmallPicImg;
				case "IsShow" :
					return model.IsShow;
				case "IsPage" :
					return model.IsPage;
		...
		#region 更新InformationClass表指定字段值
		/// <summary>更新InformationClass表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<InformationClass>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了InformationClass表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新InformationClass表指定主键Id的字段值
		/// <summary>更新InformationClass表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了InformationClass表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationClassTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新InformationClass表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了InformationClass表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新InformationClass表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了InformationClass表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        public string GetName(Page page, int pkValue, bool isCache = true)
        {
            ...
            {
                //从数据库中查询
                var model = InformationClass.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.Name;
            }
        }
        #endregion
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateIsSys(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[InformationClassTable.IsSys] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了InformationClass表id为【" + pkValue + "】的记录，更新内容为将IsSys字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		
		#region 删除BigPicImg字段存储的对应图片
		/// <summary>删除BigPicImg字段存储的对应图片</summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="pkValue">主键Id</param>
        /// <param name="isCache">是否同步更新缓存</param>
		public void DelBigPicImg(Page page, int pkValue, bool isCache = true) {
			try {
				string img = "";

				//设置条件
				var wheres = new List<ConditionFun.SqlqueryCondition>();
				wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, InformationClassTable.Id, Comparison.Equals, pkValue));

				//获取图片地址
				var select = new SelectHelper();
				img = select.GetColumnsValue<InformationClass>(InformationClassTable.BigPicImg, wheres) + "";
				...
				//设置更新值
				var setValue = new Dictionary<string, object>();
				setValue[InformationClassTable.BigPicImg] = "";
				//更新
                ...
                if (isCache && CommonBll.IsUseCache())
                {
                    ...
                    var model = GetModelForCache(pkValue);
                    //给获取的实体赋值
                    SetModelValue(model, InformationClassTable.BigPicImg, "");
                    //更新缓存中的实体
                    SetModelForCache(model);
                }
			}
		...
		#region 删除SmallPicImg字段存储的对应图片
		/// <summary>删除SmallPicImg字段存储的对应图片</summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="pkValue">主键Id</param>
        /// <param name="isCache">是否同步更新缓存</param>
		public void DelSmallPicImg(Page page, int pkValue, bool isCache = true) {
			try {
				string img = "";
				...
				//获取图片地址
				var select = new SelectHelper();
				img = select.GetColumnsValue<InformationClass>(InformationClassTable.SmallPicImg, wheres) + "";
				...
				//设置更新值
				var setValue = new Dictionary<string, object>();
				setValue[InformationClassTable.SmallPicImg] = "";
				//更新
				UpdateValue(page, setValue, wheres, "{0}更新了Ad表id为【" + pkValue + "】的记录，将图片Img删除", false);
                ...
                if (isCache && CommonBll.IsUseCache())
                {
                    ...
                    var model = GetModelForCache(pkValue);
                    //给获取的实体赋值
                    SetModelValue(model, InformationClassTable.SmallPicImg, "");
                    //更新缓存中的实体
                    SetModelForCache(model);
                }
			}
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateIsShow(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[InformationClassTable.IsShow] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了InformationClass表id为【" + pkValue + "】的记录，更新内容为将IsShow字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public void UpdateIsPage(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[InformationClassTable.IsPage] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了InformationClass表id为【" + pkValue + "】的记录，更新内容为将IsPage字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		
		#region 使用SeoKey来查询，获取一个InformationClass实体对象
        /// <summary>使用Key来查询，获取一个InformationClass实体对象</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="key">Key值</param>
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns>DataAccess.Model.InformationClass 实体</returns>
        public DataAccess.Model.InformationClass GetModel_BySeoKey(Page page, string key, bool isCache = true)
        {
			...
			{
				//从数据库中查询
				return Transform(InformationClass.SingleOrDefault(x => x.SeoKey == key));
			}
        }
        #endregion
