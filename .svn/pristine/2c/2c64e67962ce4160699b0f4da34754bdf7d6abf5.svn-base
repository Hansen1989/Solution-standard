...
namespace Solution.Logic.Managers
{
    ...
    public partial class BranchBll : LogicBase
    {
        ...
        private const string const_CacheKey_Model = "Cache_Branch_AllModel";

        #region 获取Branch全表内容并放到缓存中

        /// <summary>
        /// 取得Branch全表内容——使用菜单地址做为KEY
        /// </summary>
        ...
        public Hashtable GetHashtable()
        {
            ...
            if (obj == null)
            {
                ...
                foreach (var model in all)
                {
                    ...
                    branch.Sort = model.Sort;
                    branch.Depth = model.Depth;
                    branch.Manager_Id = model.Manager_Id;
                    branch.Manager_CName = model.Manager_CName;
                    branch.UpdateDate = model.UpdateDate;

                    try
                    {
                        //将菜单实体存入容器中
                        //使用页面地址做为KEY
                        ht.Add(branch.Code, branch);
                    }
                    catch (Exception)
                    {
                    }
        ...
        /// 根据菜单Url地址，获取菜单相应内容
        /// </summary>
        /// <param name="BranchUrl">页面地址</param>
        /// <returns>返回菜单实体</returns>
        public Branch GetBranch(string BranchUrl)
        {
            try
            {
                //从全局缓存中读取菜单内容
                //获取菜单实体
                return (Branch)(BranchBll.GetInstence().GetHashtable()[BranchUrl]);
            }
            catch (Exception)
            {
                return new Branch();
        ...
        /// 绑定菜单下拉列表——显示所有的数据
        /// </summary>
        public void BandDropDownListShowName(Page page, System.Web.UI.WebControls.DropDownList ddl)
        {

            //在内存中筛选记录
            var dt = DataTableHelper.GetFilterData(GetDataTable(), "1", "1", BranchTable.Id, " asc");
