...
namespace Solution.Logic.Managers {
	...
	public partial class TABLE_SEEDBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_TABLE_SEED";
        private const string const_CacheKey_Date = "Cache_TABLE_SEED_Date";

		#region 单例模式
		//定义单例实体
		private static TABLE_SEEDBll _TABLE_SEEDBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static TABLE_SEEDBll GetInstence() {
			if (_TABLE_SEEDBll == null) {
				_TABLE_SEEDBll = new TABLE_SEEDBll();
			}
			return _TABLE_SEEDBll;
		}
		#endregion
		...
		#region 从IIS缓存中获取TABLE_SEED表记录
		/// <summary>
        /// 从IIS缓存中获取TABLE_SEED表记录
        /// </summary>
	    /// <param name="isCache">是否从缓存中读取</param>
        public IList<DataAccess.Model.TABLE_SEED> GetList(bool isCache = true)
        {
			try
			{
				//判断是否使用缓存
				if (CommonBll.IsUseCache() && isCache){
					...
					{
						return (IList<DataAccess.Model.TABLE_SEED>)obj;
					}
				}
				else
				{
					//定义临时实体集
					IList<DataAccess.Model.TABLE_SEED> list = null;

					//获取全表缓存加载条件表达式
					var exp = GetExpression<TABLE_SEED>();
                    //如果条件为空，则查询全表所有记录
					if (exp == null)
					{
						//从数据库中获取所有记录
						var all = TABLE_SEED.All();
                        list = all == null ? null : Transform(all.ToList());
					}
					else
					{
                        //从数据库中查询出指定条件的记录，并转换为指定实体集
						var all = TABLE_SEED.Find(exp);
        ...
        public DataAccess.Model.TABLE_SEED GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = TABLE_SEED.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.TABLE_SEED</returns>
        public DataAccess.Model.TABLE_SEED GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = TABLE_SEED.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.TABLE_SEED</returns>
        public DataAccess.Model.TABLE_SEED GetModelForCache(Expression<Func<DataAccess.Model.TABLE_SEED, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.TABLE_SEED model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(TABLE_SEED model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中TABLE_SEED表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.TABLE_SEED, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将TABLE_SEED记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.TABLE_SEED）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.TABLE_SEED</returns>
		public DataAccess.Model.TABLE_SEED Transform(TABLE_SEED model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.TABLE_SEED
            {
                Id = model.Id,
                TABLE_NAME = model.TABLE_NAME,
                SEED_DATETIME = model.SEED_DATETIME,
                SEED_ID = model.SEED_ID,
            };
        }

		/// <summary>
		/// 将TABLE_SEED记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.TABLE_SEED）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.TABLE_SEED> Transform(IList<TABLE_SEED> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.TABLE_SEED>();
		...
		/// <summary>
		/// 将TABLE_SEED记录实体由普通的实体（DataAccess.Model.TABLE_SEED）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.TABLE_SEED）</param>
		/// <returns>TABLE_SEED</returns>
		public TABLE_SEED Transform(DataAccess.Model.TABLE_SEED model)
        {
			if (model == null) 
				return null;

            return new TABLE_SEED
            {
                Id = model.Id,
                TABLE_NAME = model.TABLE_NAME,
                SEED_DATETIME = model.SEED_DATETIME,
		...
		/// <summary>
		/// 将TABLE_SEED记录实体由普通实体集（DataAccess.Model.TABLE_SEED）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.TABLE_SEED）</param>
        public IList<TABLE_SEED> Transform(IList<DataAccess.Model.TABLE_SEED> sourceList)
        {
			//创建List容器
            var list = new List<TABLE_SEED>();
		...
		public void SetModelValue(DataAccess.Model.TABLE_SEED model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.TABLE_SEED model, string colName, object value)
		{
            ...
            switch (colName)
            {
				case "Id" :
                    ...
                    break;
				case "TABLE_NAME" :
					model.TABLE_NAME = (string)value;
                    break;
				case "SEED_DATETIME" :
					model.SEED_DATETIME = (string)value;
                    break;
				case "SEED_ID" :
					model.SEED_ID = (int)value;
                    break;
            }
		}
		...
		/// <summary>
		/// 获取TABLE_SEED表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<TABLE_SEED>(wheres);

		}

		/// <summary>
		/// 获取TABLE_SEED表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<TABLE_SEED, bool>> expression) {
            return new Select().From<TABLE_SEED>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.TABLE_SEED> Find(Expression<Func<DataAccess.Model.TABLE_SEED, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return TABLE_SEED.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.TABLE_SEED, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<TABLE_SEED>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取TABLE_SEED表记录时出现异常", e);
		...
		#region 添加与编辑TABLE_SEED表记录
		/// <summary>
		/// 添加与编辑TABLE_SEED记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">TABLE_SEED表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, TABLE_SEED model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "TABLE_SEED记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行TABLE_SEEDBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", TABLE_SEEDTable.TableName,  TABLE_SEEDTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了TABLE_SEED表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", TABLE_SEEDTable.TableName,  TABLE_SEEDTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了TABLE_SEED表id为【" + str + "】的记录！");
			}
		}
        ...
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<TABLE_SEED, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			TABLE_SEED.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了TABLE_SEED表记录！");
			}
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "TABLE_SEED", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了TABLE_SEED表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "TABLE_SEED", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对TABLE_SEED表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = TABLE_SEEDTable.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.TABLE_SEED, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<TABLE_SEED>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.TABLE_SEED model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "SEED_DATETIME" :
					return model.SEED_DATETIME;
				case "SEED_ID" :
					return model.SEED_ID;
			}
		...
		#region 更新TABLE_SEED表指定字段值
		/// <summary>更新TABLE_SEED表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<TABLE_SEED>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了TABLE_SEED表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新TABLE_SEED表指定主键Id的字段值
		/// <summary>更新TABLE_SEED表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了TABLE_SEED表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, TABLE_SEEDTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新TABLE_SEED表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了TABLE_SEED表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新TABLE_SEED表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了TABLE_SEED表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
