...
namespace Solution.Logic.Managers {
	...
	public partial class SHOP00Bll : LogicBase {
		...
		private const string const_CacheKey = "Cache_SHOP00";
        private const string const_CacheKey_Date = "Cache_SHOP00_Date";

		#region 单例模式
		//定义单例实体
		private static SHOP00Bll _SHOP00Bll = null;
        ...
        /// <returns>DataAccess.Model.SHOP00</returns>
        public DataAccess.Model.SHOP00 GetModelForCache(Expression<Func<DataAccess.Model.SHOP00, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.SHOP00 model)
        {
            //从缓存中读取记录列表
            var list = GetList();
            //从缓存中删除记录
		...
		/// <summary>
        /// 按条件删除IIS缓存中SHOP00表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.SHOP00, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将SHOP00记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.SHOP00）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.SHOP00> Transform(IList<SHOP00> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.SHOP00>();
		...
		/// <summary>
		/// 将SHOP00记录实体由普通的实体（DataAccess.Model.SHOP00）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.SHOP00）</param>
		/// <returns>SHOP00</returns>
		public SHOP00 Transform(DataAccess.Model.SHOP00 model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将SHOP00记录实体由普通实体集（DataAccess.Model.SHOP00）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.SHOP00）</param>
        public IList<SHOP00> Transform(IList<DataAccess.Model.SHOP00> sourceList)
        {
			//创建List容器
            var list = new List<SHOP00>();
		...
		public void SetModelValue(DataAccess.Model.SHOP00 model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.SHOP00 model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "SHOP_Price_Area" :
                    ...
                    break;
				case "SHOP_ADD" :
					model.SHOP_ADD = (string)value;
                    break;
				case "SHOP_TEL" :
					model.SHOP_TEL = (string)value;
                    break;
				case "SHOP_EMAIL" :
					model.SHOP_EMAIL = (string)value;
                    break;
				case "SHOP_CONTECT" :
					model.SHOP_CONTECT = (string)value;
                    break;
				case "SHOP_MEMO" :
					model.SHOP_MEMO = (string)value;
                    break;
				case "SHOP_STATUS" :
					model.SHOP_STATUS = ConvertHelper.Ctinyint(value);
                    break;
				case "SHOP_Limited" :
					model.SHOP_Limited = (int)value;
                    break;
				case "CRT_DATETIME" :
		...
		/// <summary>
		/// 获取SHOP00表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionFun.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<SHOP00>(wheres);

		}

		/// <summary>
		/// 获取SHOP00表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<SHOP00, bool>> expression) {
            return new Select().From<SHOP00>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.SHOP00> Find(Expression<Func<DataAccess.Model.SHOP00, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.SHOP00, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionFun.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<SHOP00>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取SHOP00表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", SHOP00Table.TableName,  SHOP00Table.Id, id);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了SHOP00表id为【" + id + "】的记录！");
			}
		}
		...
		public override void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", SHOP00Table.TableName,  SHOP00Table.Id, str);
		    ...
		    {
				//添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}删除了SHOP00表id为【" + str + "】的记录！");
			}
		}
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "SHOP00", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}更新了SHOP00表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "SHOP00", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				UseLogBll.GetInstence().Save(page, "{0}对SHOP00表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
            ...
            if (string.IsNullOrEmpty(conditionColName))
                conditionColName = SHOP00Table.Id;
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.SHOP00, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<SHOP00>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.SHOP00 model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "SHOP_NAME2" :
					return model.SHOP_NAME2;
				case "SHOP_KIND" :
					return model.SHOP_KIND;
				case "SHOP_Area_ID" :
					return model.SHOP_Area_ID;
				case "SHOP_Price_Area" :
					return model.SHOP_Price_Area;
				case "SHOP_ADD" :
					return model.SHOP_ADD;
				case "SHOP_TEL" :
					return model.SHOP_TEL;
				case "SHOP_EMAIL" :
					return model.SHOP_EMAIL;
				case "SHOP_CONTECT" :
					return model.SHOP_CONTECT;
				case "SHOP_MEMO" :
					return model.SHOP_MEMO;
				case "SHOP_STATUS" :
					return model.SHOP_STATUS;
				case "SHOP_Limited" :
					return model.SHOP_Limited;
				case "CRT_DATETIME" :
		...
		#region 更新SHOP00表指定字段值
		/// <summary>更新SHOP00表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionFun.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<SHOP00>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了SHOP00表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新SHOP00表指定主键Id的字段值
		/// <summary>更新SHOP00表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了SHOP00表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionFun.SqlqueryCondition(ConstraintType.And, SHOP00Table.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新SHOP00表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了SHOP00表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新SHOP00表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了SHOP00表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
		public void UpdateSHOP_STATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[SHOP00Table.SHOP_STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了SHOP00表id为【" + pkValue + "】的记录，更新内容为将SHOP_STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
		...
		public void UpdateSTATUS(Page page, int pkValue, int updateValue, bool isCache = true, bool isAddUseLog = true) {
			//设置更新值
			var setValue = new Dictionary<string, object>();
			setValue[SHOP00Table.STATUS] = updateValue;

			//更新
			UpdateValue(page, pkValue, setValue, "{0}更新了SHOP00表id为【" + pkValue + "】的记录，更新内容为将STATUS字段值修改为" + updateValue, isCache, isAddUseLog);
		}
		#endregion
