<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ProdKindList.aspx.cs" Inherits="Solution.Web.Managers.WebManage.Systems.DataCenter.ProdKindList" %>
...
<body>
    ...
    <f:panel id="Panel1" runat="server" title="菜单(页面)列表" enableframe="false" bodypadding="10px"
        ...
        <items>
            <f:Grid ID="Grid1" Title="商品性质(大类)列表" EnableFrame="false" EnableCollapse="true" AllowSorting="true" SortField="Depth" SortDirection="ASC" 
            ...
            OnPageIndexChange="Grid1_PageIndexChange" OnPreRowDataBound="Grid1_PreRowDataBound" OnRowCommand="Grid1_RowCommand" > 
                <Columns>
                    <f:RowNumberField />
                    <f:BoundField Width="180px" DataField="KIND_ID" HeaderText="性质编码" />
                    <f:BoundField DataField="KIND_NAME" HeaderText="性质名称" />
                    <f:LinkButtonField HeaderText="是否有效" Icon="BulletCross" TextAlign="Center" ToolTip="点击修改是否有效" ColumnID="ENABLE" CommandName="ENABLE" />
                    <f:LinkButtonField HeaderText="是否有配方" Icon="BulletCross" TextAlign="Center" ToolTip="点击修改是否有配方" ColumnID="ISBOM" CommandName="ISBOM" />
                    <f:BoundField DataField="KIND_MEMO" HeaderText="备注" ExpandUnusedSpace="true" />
