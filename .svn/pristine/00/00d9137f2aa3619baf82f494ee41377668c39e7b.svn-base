<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ProdDepList.aspx.cs" Inherits="Solution.Web.Managers.WebManage.Systems.DataCenter.ProdDepList" %>
...
<body>
    ...
    <f:panel id="Panel1" runat="server" title="菜单(页面)列表" enableframe="false" bodypadding="10px"
        ...
        <items>
            <f:Grid ID="Grid1" Title="商品类别(中类)列表" EnableFrame="false" EnableCollapse="true" AllowSorting="true" SortField="Depth" SortDirection="ASC" 
            ...
            OnPageIndexChange="Grid1_PageIndexChange" OnPreRowDataBound="Grid1_PreRowDataBound" OnRowCommand="Grid1_RowCommand"> 
                <Columns>
                    <f:RowNumberField />
                    <f:BoundField Width="180px" DataField="DEP_ID" HeaderText="类别编码" />
                    <f:BoundField DataField="DEP_NAME" HeaderText="类别名称" />
                    <f:LinkButtonField HeaderText="是否有效" Icon="BulletCross" TextAlign="Center" ToolTip="点击修改是否有效" ColumnID="ENABLE" CommandName="ENABLE" />
                    <f:BoundField DataField="KIND_ID" HeaderText="隶属大类内码" runat="server" />
                    <f:BoundField ID="KIND_NAME" DataField="KIND_NAME" HeaderText="隶属大类名称" runat="server" />
                    <%--<f:RenderField ExpandUnusedSpace="true" FieldType="String"
                    HeaderText="隶属大类名称">
                    <Editor>
                        <f:DropDownList ID="sKIND_NAME" Required="true" runat="server">
                        </f:DropDownList>
                    </Editor>
                    </f:RenderField>--%>

                     <%--<f:TemplateField HeaderText="隶属大类内码">
                        <ItemTemplate>
                            <asp:DropDownList runat="server" ID="sKIND_NAME">
                            </asp:DropDownList>
                        </ItemTemplate>
                    </f:TemplateField> DataField="KIND_ID" ColumnID="KIND_ID"  ID="ssK_name" --%>
                    <f:BoundField DataField="DEP_MEMO" HeaderText="备注" ExpandUnusedSpace="true" />
