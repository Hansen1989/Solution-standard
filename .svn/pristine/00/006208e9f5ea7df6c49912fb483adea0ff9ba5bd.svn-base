<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ProdCateList.aspx.cs" Inherits="Solution.Web.Managers.WebManage.Systems.DataCenter.ProdCateList" %>
...
<form id="form1" runat="server">
    ...
    <f:panel id="Panel1" runat="server" title="菜单(页面)列表" enableframe="false" bodypadding="10px"
        ...
        <items>
            <f:Grid ID="Grid1" Title="商品小类列表" EnableFrame="false" EnableCollapse="true" AllowSorting="true" SortField="Depth" SortDirection="ASC" 
            ...
            OnPageIndexChange="Grid1_PageIndexChange" OnPreRowDataBound="Grid1_PreRowDataBound" OnRowCommand="Grid1_RowCommand" > 
                <Columns>
                    <f:RowNumberField />
                    <f:BoundField Width="180px" DataField="CATE_ID" HeaderText="类别编号" />
                    <f:BoundField DataField="CATE_NAME" HeaderText="类别名称" />
                    <f:LinkButtonField HeaderText="是否有效" Icon="BulletCross" TextAlign="Center" ToolTip="点击修改是否有效" ColumnID="ENABLE" CommandName="ENABLE" />
                    <f:BoundField DataField="DEP_ID" HeaderText="隶属中类编码" />
                    <f:BoundField DataField="DEP_NAME" HeaderText="隶属中类名称" />
                    <%-- <f:TemplateField HeaderText="隶属中类名称">
                        <ItemTemplate>
                            <asp:DropDownList runat="server" ID="DEP_NAME">
                            </asp:DropDownList>
                        </ItemTemplate>
                    </f:TemplateField>--%>
                    <f:BoundField DataField="CATE_MEMO" HeaderText="备注" ExpandUnusedSpace="true" />
